...
$agentcallsstatus		= '0';	# set to 1 to show agent status and call dialed count
   $campagentstatctmax	= '3';	# Number ofсекунды for campaign call and agent stats
...
$PreseT_DiaL_LinKs		= '0';	# set to 1 to show a НАБОР НОМЕРА link for Dial Presets
...
$hangup_all_non_reserved= '1';	# set to 1 to force hangup all non-reserved channels upon Разъединить Клиента
$LogouTKicKAlL			= '1';	# set to 1 to hangup all calls in session upon agent logout
$ТелефонSComPIP			= '1';	# set to 1 to log computer IP to phone if blank, set to 2 to force log each login
$DefaulTAlTDiaL			= '0';	# set to 1 to enable ALT НАБОР НОМЕРА by default if enabled for the campaign
			...
			if (xmlhttp) 
				...
				xmlhttp.onreadystatechange = function() 
					...
					//	alert(xmlhttp.responseText);
						...
						document.getElementById("LogiNReseT").innerHTML = "<INPUT TYPE=BUTTON VALUE=\"Обновление Кампания List\" OnClick=\"login_allowable_campaigns()\">";
...
if ($relogin == 'YES')
{
echo "<title>Agent web client: Re-Логин</title>\n";
echo "</head>\n";
...
echo "<TD ALIGN=CENTER VALIGN=MIDDLE> Re-Логин </TD>";
echo "</TR>\n";
...
echo "<span id=\"LogiNReseT\"><INPUT TYPE=BUTTON VALUE=\"Обновление Кампания List\" OnClick=\"login_allowable_campaigns()\"></span></TD></TR>\n";
...
if ($user_login_first == 1)
{
	...
	{
	echo "<title>Agent web client: Логин Кампании</title>\n";
	echo "</head>\n";
	echo "<BODY BGCOLOR=WHITE MARGINHEIGHT=0 MARGINWIDTH=0 onResize=\"browser_dimensions();\"  onLoad=\"browser_dimensions();\">\n";
	echo "<A HREF=\"./timeclock.php?referrer=agent&pl=$phone_login&pp=$phone_pass&VD_login=$VD_login&VD_pass=$VD_pass\">Timeclock</A><BR>\n";
	echo "<TABLE WIDTH=100%><TR><TD></TD>\n";
...
echo "<TD WIDTH=100 ALIGN=RIGHT VALIGN=TOP  NOWRAP><a href=\"../agc_en/vicidial.php?relogin=YES&VD_login=$VD_login&VD_campaign=$VD_campaign&phone_login=$phone_login&phone_pass=$phone_pass&VD_pass=$VD_pass\">English <img src=\"../agc/images/en.gif\" BORDER=0 HEIGHT=14 WIDTH=20></a></TD>\n";echo "<TD WIDTH=100 ALIGN=RIGHT VALIGN=TOP  BGCOLOR=\"#CCFFCC\" NOWRAP><a href=\"../agc_ru/vicidial.php?relogin=YES&VD_login=$VD_login&VD_campaign=$VD_campaign&phone_login=$phone_login&phone_pass=$phone_pass&VD_pass=$VD_pass\">Russian <img src=\"../agc/images/ru.gif\" BORDER=0 HEIGHT=14 WIDTH=20></a></TD>\n";	echo "</TR></TABLE>\n";
	...
	echo "<CENTER><BR><B>Логин Пользователя</B><BR><BR>";
	echo "<TABLE WIDTH=460 CELLPADDING=0 CELLSPACING=0 BGCOLOR=\"$MAIN_COLOR\"><TR BGCOLOR=WHITE>";
	echo "<TD ALIGN=LEFT VALIGN=BOTTOM><IMG SRC=\"../agc/images/vdc_tab_vicidial.gif\" Border=0></TD>";
	echo "<TD ALIGN=CENTER VALIGN=MIDDLE> Логин Кампании </TD>";
	echo "</TR>\n";
	echo "<TR><TD ALIGN=LEFT COLSPAN=2><font size=1> &nbsp; </TD></TR>\n";
	echo "<TR><TD ALIGN=RIGHT>Логин Пользователя:  </TD>";
	echo "<TD ALIGN=LEFT><INPUT TYPE=TEXT NAME=VD_login SIZE=10 maxlength=20 VALUE=\"$VD_login\"></TD></TR>\n";
	...
	else
	{
		...
		echo "<title>Agent web client: Логин</title>\n";
		echo "</head>\n";
		echo "<BODY BGCOLOR=WHITE MARGINHEIGHT=0 MARGINWIDTH=0 onResize=\"browser_dimensions();\"  onLoad=\"browser_dimensions();\">\n";
		echo "<A HREF=\"./timeclock.php?referrer=agent&pl=$phone_login&pp=$phone_pass&VD_login=$VD_login&VD_pass=$VD_pass\">Timeclock</A><BR>\n";
		echo "<TABLE WIDTH=100%><TR><TD></TD>\n";
...
if ( (strlen($phone_login)<2) or (strlen($phone_pass)<2) )
{
echo "<title>Agent web client:  Логин Телефона</title>\n";
echo "</head>\n";
echo "<BODY BGCOLOR=WHITE MARGINHEIGHT=0 MARGINWIDTH=0 onResize=\"browser_dimensions();\"  onLoad=\"browser_dimensions();\">\n";
echo "<A HREF=\"./timeclock.php?referrer=agent&pl=$phone_login&pp=$phone_pass&VD_login=$VD_login&VD_pass=$VD_pass\">Timeclock</A><BR>\n";
echo "<TABLE WIDTH=100%><TR><TD></TD>\n";
	...
	if($auth>0)
		...
		if ( (ereg('Y',$forced_timeclock_login)) or ( (ereg('ADMIN_EXEMPT',$forced_timeclock_login)) and ($VU_user_level < 8) ) )
			...
			if ( (strlen($last_agent_event)<2) or (ereg('ВЫХОД',$last_agent_event)) )
				{
				$VDloginDISPLAY=1;
				$VDdisplayMESSAGE = "Вы должны войти в систему TIMECLOCK ПЕРВЫЙ<BR>";
				}
			}
		...
		if ( ( (ereg("СТАРТ|ALL",$shift_enforcement)) and (!ereg("OFF",$VU_agent_shift_enforcement_override)) ) or (ereg("СТАРТ|ALL",$VU_agent_shift_enforcement_override)) )
			...
			if ( (strlen($LOGgroup_shiftsSQL) < 3) and ($VU_shift_override_flag < 1) )
				{
				$VDloginDISPLAY=1;
				$VDdisplayMESSAGE = "ОШИБКА: Есть сдвиги не включена для пользователя группы<BR>";
				}
			else
				{
				$HHMM = date("Hi");
				...
				if ( ($shift_ok < 1) and ($VU_shift_override_flag < 1) )
					{
					$VDloginDISPLAY=1;
					$VDdisplayMESSAGE = "ОШИБКА: Вы не можете войти в вашу за переход<BR>";
					}
				}
			if ( ($shift_ok < 1) and ($VU_shift_override_flag < 1) and ($VDloginDISPLAY > 0) )
				...
				$VDdisplayMESSAGE.= "МенеджерЛогин: <INPUT TYPE=TEXT NAME=\"MGR_login$loginDATE\" SIZE=10 maxlength=20><br>\n";
				$VDdisplayMESSAGE.= "МенеджерПароль: <INPUT TYPE=PASSWORD NAME=\"MGR_pass$loginDATE\" SIZE=10 maxlength=20><br>\n";
...
echo "<TD WIDTH=100 ALIGN=RIGHT VALIGN=TOP  NOWRAP><a href=\"../agc_en/vicidial.php?relogin=YES&VD_login=$VD_login&VD_campaign=$VD_campaign&phone_login=$phone_login&phone_pass=$phone_pass&VD_pass=$VD_pass\">English <img src=\"../agc/images/en.gif\" BORDER=0 HEIGHT=14 WIDTH=20></a></TD>\n";echo "<TD WIDTH=100 ALIGN=RIGHT VALIGN=TOP  BGCOLOR=\"#CCFFCC\" NOWRAP><a href=\"../agc_ru/vicidial.php?relogin=YES&VD_login=$VD_login&VD_campaign=$VD_campaign&phone_login=$phone_login&phone_pass=$phone_pass&VD_pass=$VD_pass\">Russian <img src=\"../agc/images/ru.gif\" BORDER=0 HEIGHT=14 WIDTH=20></a></TD>\n";			echo "</TR></TABLE>\n";
			...
			echo "<INPUT TYPE=HIDDEN NAME=phone_login VALUE=\"$phone_login\">\n";
			echo "<INPUT TYPE=HIDDEN NAME=phone_pass VALUE=\"$phone_pass\">\n";
			echo "Логин: <INPUT TYPE=TEXT NAME=VD_login SIZE=10 maxlength=20 VALUE=\"$VD_login\">\n<br>";
		...
		if($CAMPactive>0)
			...
			if ( (!ereg('DISABLED',$VU_vicidial_recording_override)) and ($VU_vicidial_recording > 0) )
				...
				echo "<!-- USER ЗАПИСЬING OVERRIDE: |$VU_vicidial_recording_override|$campaign_recording| -->\n";
				}
			...
			$row=mysql_fetch_row($rslt);
			   ...
			   echo "<!-- $campaign_leads_to_call - Leads оставил вызовы в контейнере -->\n";

			}
		else
			{
			$VDloginDISPLAY=1;
			$VDdisplayMESSAGE = "Кампания не активна, пожалуйста, попробуйте еще раз<BR>";
			}
		}
	else
		{
		...
		$VDloginDISPLAY=1;
		$VDdisplayMESSAGE = "Логин неправильный, пожалуйста, попробуйте еще раз<BR>";
		}
	}
	if ($VDloginDISPLAY)
	{
...
if (!$authphone)
	{
	echo "<title>Agent web client: Логин Телефона Error</title>\n";
	echo "</head>\n";
	echo "<BODY BGCOLOR=WHITE MARGINHEIGHT=0 MARGINWIDTH=0 onResize=\"browser_dimensions();\"  onLoad=\"browser_dimensions();\">\n";
	echo "<A HREF=\"./timeclock.php?referrer=agent&pl=$phone_login&pp=$phone_pass&VD_login=$VD_login&VD_pass=$VD_pass\">Timeclock</A><BR>\n";
	echo "<TABLE WIDTH=100%><TR><TD></TD>\n";
...
echo "<TD WIDTH=100 ALIGN=RIGHT VALIGN=TOP  NOWRAP><a href=\"../agc_en/vicidial.php?relogin=YES&VD_login=$VD_login&VD_campaign=$VD_campaign&phone_login=$phone_login&phone_pass=$phone_pass&VD_pass=$VD_pass\">English <img src=\"../agc/images/en.gif\" BORDER=0 HEIGHT=14 WIDTH=20></a></TD>\n";echo "<TD WIDTH=100 ALIGN=RIGHT VALIGN=TOP  BGCOLOR=\"#CCFFCC\" NOWRAP><a href=\"../agc_ru/vicidial.php?relogin=YES&VD_login=$VD_login&VD_campaign=$VD_campaign&phone_login=$phone_login&phone_pass=$phone_pass&VD_pass=$VD_pass\">Russian <img src=\"../agc/images/ru.gif\" BORDER=0 HEIGHT=14 WIDTH=20></a></TD>\n";	echo "</TR></TABLE>\n";
	...
	echo "<BR><BR><BR><CENTER><TABLE WIDTH=460 CELLPADDING=0 CELLSPACING=0 BGCOLOR=\"$MAIN_COLOR\"><TR BGCOLOR=WHITE>";
	echo "<TD ALIGN=LEFT VALIGN=BOTTOM><IMG SRC=\"../agc/images/vdc_tab_vicidial.gif\" Border=0></TD>";
	echo "<TD ALIGN=CENTER VALIGN=MIDDLE> Логин Error</TD>";
	echo "</TR>\n";
...
### logged in and use that phone login account
	...
	$ВызывающийID_popup_enabled=$row[48];
	...
	if ( (strlen($park_ext)>0) && (strlen($park_file_name)>0) )
		...
		echo "<!-- ПОЛЬЗОВАТЕЛЬСКИЙ РЕЖИМ ПАРКОВКИ КАМПАНИИ:  |$VICIDiaL_park_on_extension|$VICIDiaL_park_on_filename| -->\n";
		}
	echo "<!-- РЕЖИМ ПАРКОВКИ КАМПАНИИ ПО УМОЛЧАНИЮ: |$VICIDiaL_park_on_extension|$VICIDiaL_park_on_filename| -->\n";
	...
	if (strlen($web_form_address)>0)
		...
		echo "<!-- ПОЛЬЗОВАТЕЛЬСКАЯ WEB-ФОРМА КАМПАНИИ:   |$VICIDiaL_web_form_address| -->\n";
		}
	else
		{
		$VICIDiaL_web_form_address = "$VICIDiaL_web_URL";
		print "<!-- WEB-ФОРМА КАМПАНИИ ПО УМОЛЧАНИЮ:  |$VICIDiaL_web_form_address| -->\n";
	...
	if (strlen($web_form_address_two)>0)
		...
		echo "<!-- ПОЛЬЗОВАТЕЛЬСКАЯ WEB-ФОРМА КАМПАНИИ 2:   |$VICIDiaL_web_form_address_two| -->\n";
		}
	else
		{
		$VICIDiaL_web_form_address_two = "$VICIDiaL_web_form_address";
		echo "<!-- WEB-ФОРМА КАМПАНИИ ПО УМОЛЧАНИЮ 2:  |$VICIDiaL_web_form_address_two| -->\n";
	...
	if ($allow_closers=="Y")
		...
		echo "<!-- КАМПАНИЯ ПОЗВОЛЯЕТ CLOSERS:    |$VICIDiaL_allow_closers| -->\n";
		}
	else
		{
		$VICIDiaL_allow_closers = 0;
		echo "<!-- КАМПАНИЯ НЕ ПОЗВОЛЯЕТ CLOSERS: |$VICIDiaL_allow_closers| -->\n";
		}
	...
	if ( ( ($campaign_allow_inbound == 'Y') and ($dial_method != 'MANUAL') ) || ($campaign_leads_to_call > 0) || (ereg('Y',$no_hopper_leads_logins)) )
		...
		if ($prev_login_ct > 0)
			{echo "<!-- ИСПОЛЬЗОВАНИЕ ПРЕДЫДУЩЕЙ MEETME ROOM - $session_id - $NOW_TIME - $SIP_user -->\n";}
		else
			{
			...
			echo "<!-- ИСПОЛЬЗОВАНИЕ НОВОЙ MEETME ROOM - $session_id - $NOW_TIME - $SIP_user -->\n";
			}
		...
		$affected_rows = mysql_affected_rows($link);
		echo "<!-- старые отчеты vicidial_live_agents очищены: |$affected_rows| -->\n";
	...
	#	echo "<B>You have logged in as user: $VD_login на phone: $SIP_user в кампанию: $VD_campaign</B><BR>\n";
		...
		if ($auto_dial_level > 0)
			{
			echo "<!-- кампания установлена в auto_dial_level: $auto_dial_level -->\n";
			...
			$affected_rows = mysql_affected_rows($link);
			echo "<!-- новая vicidial_live_agents запись вставлено: |$affected_rows| -->\n";
			...
			if ($mel > 0) {mysql_error_logging($NOW_TIME,$linkB,$mel,$stmt,'01045',$VD_login,$server_ip,$session_name,$one_mysql_log);}
				...
				$stmt = "INSERT INTO queue_log SET partition='P01',time_id='$StarTtimE',call_id='NONE',queue='NONE',agent='Agent/$VD_login',verb='ПАУЗПАALL',serverid='$queuemetrics_log_id';";
				if ($DB) {echo "$stmt\n";}
				$rslt=mysql_query($stmt, $linkB);
			if ($mel > 0) {mysql_error_logging($NOW_TIME,$linkB,$mel,$stmt,'01046',$VD_login,$server_ip,$session_name,$one_mysql_log);}
				$affected_rows = mysql_affected_rows($linkB);
				echo "<!-- queue_log ПАУЗПА entry added: $VD_login|$affected_rows -->\n";
			...
			{
			print "<!-- кампания установлена в ручной набор номера: $auto_dial_level -->\n";
		...
		echo "<!-- ILPV -->\n";
echo "<TD WIDTH=100 ALIGN=RIGHT VALIGN=TOP  NOWRAP><a href=\"../agc_en/vicidial.php?relogin=YES&VD_login=$VD_login&VD_campaign=$VD_campaign&phone_login=$phone_login&phone_pass=$phone_pass&VD_pass=$VD_pass\">English <img src=\"../agc/images/en.gif\" BORDER=0 HEIGHT=14 WIDTH=20></a></TD>\n";echo "<TD WIDTH=100 ALIGN=RIGHT VALIGN=TOP  BGCOLOR=\"#CCFFCC\" NOWRAP><a href=\"../agc_ru/vicidial.php?relogin=YES&VD_login=$VD_login&VD_campaign=$VD_campaign&phone_login=$phone_login&phone_pass=$phone_pass&VD_pass=$VD_pass\">Russian <img src=\"../agc/images/ru.gif\" BORDER=0 HEIGHT=14 WIDTH=20></a></TD>\n";		echo "</TR></TABLE>\n";
		echo "<B>Жаль, но leads нет в контейнере этой кампании</B>\n";
		echo "<FORM ACTION=\"$PHP_SELF\" METHOD=POST>\n";
		echo "<INPUT TYPE=HIDDEN NAME=DB VALUE=\"$DB\">\n";
		echo "<INPUT TYPE=HIDDEN NAME=JS_browser_height VALUE=\"\">\n";
		echo "<INPUT TYPE=HIDDEN NAME=JS_browser_width VALUE=\"\">\n";
...
echo "<TD WIDTH=100 ALIGN=RIGHT VALIGN=TOP  NOWRAP><a href=\"../agc_en/vicidial.php?relogin=YES&VD_login=$VD_login&VD_campaign=$VD_campaign&phone_login=$phone_login&phone_pass=$phone_pass&VD_pass=$VD_pass\">English <img src=\"../agc/images/en.gif\" BORDER=0 HEIGHT=14 WIDTH=20></a></TD>\n";echo "<TD WIDTH=100 ALIGN=RIGHT VALIGN=TOP  BGCOLOR=\"#CCFFCC\" NOWRAP><a href=\"../agc_ru/vicidial.php?relogin=YES&VD_login=$VD_login&VD_campaign=$VD_campaign&phone_login=$phone_login&phone_pass=$phone_pass&VD_pass=$VD_pass\">Russian <img src=\"../agc/images/ru.gif\" BORDER=0 HEIGHT=14 WIDTH=20></a></TD>\n";		echo "</TR></TABLE>\n";
	...
	if ($enable_queuemetrics_logging > 0)
		...
		$stmt = "INSERT INTO queue_log SET partition='P01',time_id='$StarTtimEpause',call_id='NONE',queue='NONE',agent='Agent/$VD_login',verb='ПАУЗПАREASON',data1='LOGIN',serverid='$queuemetrics_log_id';";
		if ($DB) {echo "$stmt\n";}
		$rslt=mysql_query($stmt, $linkB);
	if ($mel > 0) {mysql_error_logging($NOW_TIME,$linkB,$mel,$stmt,'01063',$VD_login,$server_ip,$session_name,$one_mysql_log);}
		$affected_rows = mysql_affected_rows($linkB);
		echo "<!-- queue_log ПАУЗПАREASON LOGIN entry added: $VD_login|$affected_rows -->\n";
...
$SCwidth =  ($MASTERwidth + 230);	# 670 - live callсекунды counter, sidebar link
...
$Cmonths = Array('Январь','February','Март','Апрель','Май','Июнь',
...
?>
	...
	var DiaLControl_manual_HTML = "<a href=\"#\" onclick=\"ManualDialNext('','','','','','0');\"><IMG SRC=\"../agc/images/vdc_LB_dialnextnumber.gif\" border=0 alt=\"Набрать следующий номер\"></a>";
...
// filter manual dialstring and pass на to originate call
	function SendManualDial(taskFromConf)
		...
		if (taskFromConf == 'YES')
			...
			document.getElementById("DialWithCustomer").innerHTML ="<IMG SRC=\"../agc/images/vdc_XB_dialwithcustomer_OFF.gif\" border=0 alt=\"Набор номера С Клиентом\" style=\"vertical-align:middle\"></a>";

			document.getElementById("ParkCustomerDial").innerHTML ="<IMG SRC=\"../agc/images/vdc_XB_parkcustomerdial_OFF.gif\" border=0 alt=\"Парковать Набор номера Клиента\" style=\"vertical-align:middle\"></a>";
			...
			if (three_way_call_cid == 'ПОЛЬЗОВАТЕЛЬСКИЙER')
	...
	function basic_originate_call(tasknum,taskprefix,taskreverse,taskdialvalue,tasknowait,taskconfxfer,taskcid,taskusegroupalias) 
		...
		if (xmlhttp) 
			...
			xmlhttp.onreadystatechange = function() 
				...
				//		document.getElementById("HangupXferLine").innerHTML ="<a href=\"#\" onclick=\"xfercall_send_hangup();return false;\"><IMG SRC=\"../agc/images/vdc_XB_hangupxferline.gif\" border=0 alt=\"Разъединение Xfer Линии\"></a>";
						}
					}
				}
			delete xmlhttp;
				...
				xmlhttprequestcheckconf.onreadystatechange = function() 
					...
					//	alert(xmlhttprequestcheckconf.responseText);
						...
						if ( (callholdstatus == '1') || (agentcallsstatus == '1') || (vicidial_agent_disable != 'NOT_ACTIVE') )
							...
							if (AGLogiN != 'N')
								{
								document.getElementById("AgentСтатусСтатус").innerHTML = AGLogiN;
								}
							if (CamPCalLs != 'N')
								{
								document.getElementById("AgentСтатусCalls").innerHTML = CamPCalLs;
								}
							if (DiaLCalLs != 'N')
								{
								document.getElementById("AgentСтатусDiaLs").innerHTML = DiaLCalLs;
								}
							...
							if (AGLogiN == 'SHIFT_ВЫХОД')
								{
								shift_logout_flag=1;
								}
							}
						var VLAStatuS_array = check_time_array[4].split("Статус: ");
						...
						var InGroupChangeПользователь = check_time_array[13];
						...
						if (InGroupChange > 0)
							...
							var external_igb_set_user = InGroupChangeПользователь;
...
// ZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZ  above section working на API functions
						...
						if ( (conf_channels_xtra_display == 1) || (conf_channels_xtra_display == 0) )
							...
							if (live_conf_calls > 0)
								...
								agentphonelive=0;
								var conv_start=-1;
								var live_conf_HTML = "<font face=\"Arial,Helvetica\"><B>ЖИВЫЕ ВЫЗОВЫ В ВАШЕЙ СЕССИИ:</B></font><BR><TABLE WIDTH=<?php echo $CQwidth ?>><TR BGCOLOR=<?php echo $SCRIPT_COLOR ?>><TD><font class=\"log_title\">#</TD><TD><font class=\"log_title\">УДАЛЕННЫЙ КАНАЛ</TD><TD><font class=\"log_title\">РАЗЪЕДИНЕНИЕ</TD><TD><font class=\"log_title\">ГРОМКОСТЬ</TD></TR>";
	...
	function conf_send_recording(taskconfrectype,taskconfrec,taskconffile) 
		...
		if (xmlhttp) 
			...
			//	filename = filedate + "_" + user_abb;
				...
				var conf_rec_start_html = "<a href=\"#\" onclick=\"conf_send_recording('StopMonitorConf','" + taskconfrec + "','" + filename + "');return false;\"><IMG SRC=\"../agc/images/vdc_LB_stoprecording.gif\" border=0 alt=\"Остановить Запись\"></a>";

				if (LIVE_campaign_recording == 'ALLFORCE')
					{
					document.getElementById("RecorDControl").innerHTML = "<IMG SRC=\"../agc/images/vdc_LB_startrecording_OFF.gif\" border=0 alt=\"Начать Запись\">";
					}
				else
					{
			...
			if (taskconfrectype == 'StopMonitorConf')
				...
				var channelrec = "Local/" + conf_silent_prefix + '' + taskconfrec + "@" + ext_context;
				var conf_rec_start_html = "<a href=\"#\" onclick=\"conf_send_recording('MonitorConf','" + taskconfrec + "','');return false;\"><IMG SRC=\"../agc/images/vdc_LB_startrecording.gif\" border=0 alt=\"Начать Запись\"></a>";
				if (LIVE_campaign_recording == 'ALLFORCE')
					{
					document.getElementById("RecorDControl").innerHTML = "<IMG SRC=\"../agc/images/vdc_LB_startrecording_OFF.gif\" border=0 alt=\"Начать Запись\">";
					}
...
// Covers the following types: XFER, VMAIL, ENTRY, CONF, ПАРКОВКА, FROMПАРКОВКА, XfeRLOCAL, XfeRINTERNAL, XfeRBLIND, VfeRVMAIL
		...
		if (xmlhttpXF) 
			...
			if ( (taskvar == 'XfeRBLIND') || (taskvar == 'XfeRVMAIL') )
				...
				if (blindxferdialstring.length<'2')
					...
					xferredirect_query='';
					taskvar = 'NOTHING';
					alert("У номера перевода должно быть больше чем 1 цифра:" + blindxferdialstring);
					}
				else
					{
			...
			if (taskvar == 'ParK')
				...
				document.getElementById("ParkControl").innerHTML ="<a href=\"#\" onclick=\"mainxfer_send_redirect('FROMParK','" + redirectdestination + "','" + redirectdestserverip + "');return false;\"><IMG SRC=\"../agc/images/vdc_LB_grabparkedcall.gif\" border=0 alt=\"Захват Ожидающего Вызова\"></a>";
				customerparked=1;
				}
			if (taskvar == 'FROMParK')
				{
				...
				document.getElementById("ParkControl").innerHTML ="<a href=\"#\" onclick=\"mainxfer_send_redirect('ParK','" + redirectdestination + "','" + redirectdestserverip + "');return false;\"><IMG SRC=\"../agc/images/vdc_LB_parkcall.gif\" border=0 alt=\"Парковать Вызов\"></a>";
				customerparked=0;
				}
...
// Finish the alternate dialing and move на to disposition the call
	function ManualDialAltDonE()
		...
		open_dispo_screen=1;
		document.getElementById("MainStatuSSpan").innerHTML = "Набрать следующий номер";
		}
// ################################################################################
	...
	function DialLog(taskMDstage,nodeletevdac)
		...
		if (taskMDstage == "start") 
			...
			if (alt_phone_dialing == 1)
				...
				if (document.vicidial_form.DiaLAltPhonE.checked==true)
					...
					var man_status = "Набор Альт.Номера: <a href=\"#\" onclick=\"ManualDialOnly('MaiNPhonE')\"><font class=\"preview_text\">ОСНОВНОЙ ТЕЛЕФОН</font></a> or <a href=\"#\" onclick=\"ManualDialOnly('ALTТелефонE')\"><font class=\"preview_text\">АЛЬТ. ТЕЛЕФОН</font></a> or <a href=\"#\" onclick=\"ManualDialOnly('AddresS3')\"><font class=\"preview_text\">АДРЕСС 3</font></a> or <a href=\"#\" onclick=\"ManualDialAltDonE()\"><font class=\"preview_text_red\">ФИНИШ LEAD</font></a>"; 
		...
		//		document.getElementById("busycallsdebug").innerHTML = "vdc_db_query.php?" + manDiaLlog_query;
			...
			//		alert(xmlhttp.responseText);
					...
					if ( (MDlogLINE == "LOG НЕ ВВЕДЕН") && (VDstop_rec_after_each_call != 1) )
						{
				...
				//		alert("VICIDIAL Call log entered:\n" + document.vicidial_form.uniqueid.value);
						if ( (taskMDstage != "start") && (VDstop_rec_after_each_call == 1) )
							{
							var conf_rec_start_html = "<a href=\"#\" onclick=\"conf_send_recording('MonitorConf','" + session_id + "','');return false;\"><IMG SRC=\"../agc/images/vdc_LB_startrecording.gif\" border=0 alt=\"Начать Запись\"></a>";
			...
			xmlhttp.onreadystatechange = function() 
				...
				//	alert(xmlhttp.responseText);
					...
					if (CBcounT == 0) {var CBprint = "NO";}
						...
						document.getElementById("CBstatusSpan").innerHTML ="<a href=\"#\" onclick=\"CalLBacKsLisTCheck();return false;\">" + CBprint + " АКТИВНЫЕ ВЫЗОВBACK</a>";
			...
			if (xmlhttp) 
				...
				xmlhttp.onreadystatechange = function() 
					...
					//	alert(xmlhttp.responseText);
						...
						var loop_ct=0;
						var conv_start=0;
						var CB_HTML = "<table width=610><tr bgcolor=<?php echo $SCRIPT_COLOR ?>><td><font class=\"log_title\">#</td><td><font class=\"log_title\"> ВЫЗОВBACK DATE/TIME</td><td><font class=\"log_title\">НОМЕР</td><td><font class=\"log_title\">NAME</td><td><font class=\"log_title\">  СТАТУС</td><td align=right><font class=\"log_title\">CAMPAIGN</td><td><font class=\"log_title\">LAST ЗВОНИТЬ ДАТА/ВРЕМЯ</td><td align=left><font class=\"log_title\"> COMMENTS</td></tr>"
		...
		if ( (AutoDialWaiting == 1) || (VD_live_customer_call==1) || (alt_dial_active==1) )
			...
			if (TVfast=='FAST')
				...
				if (agent_allow_group_alias == 'Y')
					{
					document.getElementById("ManuaLDiaLGrouPSelecteD").innerHTML = "<font size=2 face=\"Arial,Helvetica\">Группа Alias: " + active_group_alias + "</font>";
					document.getElementById("ManuaLDiaLGrouP").innerHTML = "<a href=\"#\" onclick=\"GroupAliasSelectContent_create('0');\"><font size=1 face=\"Arial,Helvetica\">Нажмите здесь, чтобы Выбери группу Alias</font></a>";
					}
...
// Fast version of manual dial
		...
		if ( (MDDiaLCodEform.length < 1) || (MDPhonENumbeRform.length < 5) )
			{
			alert("ЧТОБЫ ИСПОЛЬЗОВАТЬ БЫСТРЫЙ НАБОР НОМЕРА, ВЫ ДОЛЖНЫ ВВЕСТИ НОМЕР ТЕЛЕФОНА И КОД СТРАНЫ");
			}
		else
			{
			var MDLookuPLeaD = 'new';
...
// Request lookup of manual dial channel
	function ManualDialCheckChanneL(taskCheckOR)
			...
			xmlhttp.onreadystatechange = function() 
				...
				//	alert(xmlhttp.responseText);
					...
					//		alert(document.getElementById("MainStatuSSpan").innerHTML);
							document.getElementById("MainStatuSSpan").innerHTML = " Запрос: " + status_display_number + " UID: " + CIDcheck + " &nbsp; Ожидание Кольца вызова... " + MD_ring_secondS + "секунды";
					//		alert("channel not found yet:\n" + campaign);
						...
						if (taskCheckOR == 'YES')
							...
							if ( (XDchannel.match(regMDL)) && (asterisk_version != '1.0.8') && (asterisk_version != '1.0.9') && (MD_ring_secondS < 10) )
								...
								document.getElementById("MainStatuSSpan").innerHTML = " Вызов третьей стороны: " + document.vicidial_form.xfernumber.value + " UID: " + CIDcheck;

								document.getElementById("Leave3WayCall").innerHTML ="<a href=\"#\" onclick=\"leave_3way_call('FIRST');return false;\"><IMG SRC=\"../agc/images/vdc_XB_leave3waycall.gif\" border=0 alt=\"ОСТАВИТЬ 3-WAY ЗАПРОС\" style=\"vertical-align:middle\"></a>";

								document.getElementById("DialWithCustomer").innerHTML ="<IMG SRC=\"../agc/images/vdc_XB_dialwithcustomer_OFF.gif\" border=0 alt=\"Набор номера С Клиентом\" style=\"vertical-align:middle\">";

								document.getElementById("ParkCustomerDial").innerHTML ="<IMG SRC=\"../agc/images/vdc_XB_parkcustomerdial_OFF.gif\" border=0 alt=\"Парковать Набор номера Клиента\" style=\"vertical-align:middle\">";

								document.getElementById("HangupXferLine").innerHTML ="<a href=\"#\" onclick=\"xfercall_send_hangup();return false;\"><IMG SRC=\"../agc/images/vdc_XB_hangupxferline.gif\" border=0 alt=\"Разъединение Xfer Линии\" style=\"vertical-align:middle\"></a>";

								document.getElementById("HangupBothLines").innerHTML ="<a href=\"#\" onclick=\"bothcall_send_hangup();return false;\"><IMG SRC=\"../agc/images/vdc_XB_hangupbothlines.gif\" border=0 alt=\"Разъединить Обе Линии\" style=\"vertical-align:middle\"></a>";
							...
							if ( (MDchannel.match(regMDL)) && (asterisk_version != '1.0.8') && (asterisk_version != '1.0.9') )
								...
								document.getElementById("MainStatuSSpan").innerHTML = " Вызванный: " + status_display_number + " UID: " + CIDcheck + " &nbsp;"; 

								document.getElementById("ParkControl").innerHTML ="<a href=\"#\" onclick=\"mainxfer_send_redirect('ParK','" + lastcustchannel + "','" + lastcustserverip + "');return false;\"><IMG SRC=\"../agc/images/vdc_LB_parkcall.gif\" border=0 alt=\"Парковать Вызов\"></a>";

								document.getElementById("HangupControl").innerHTML = "<a href=\"#\" onclick=\"dialedcall_send_hangup();\"><IMG SRC=\"../agc/images/vdc_LB_hangupcustomer.gif\" border=0 alt=\"Разъединить Клиента\"></a>";

								document.getElementById("XferControl").innerHTML = "<a href=\"#\" onclick=\"ShoWTransferMain('ON');\"><IMG SRC=\"../agc/images/vdc_LB_transferconf.gif\" border=0 alt=\"Перевод - Конференция\"></a>";

								document.getElementById("LocalCloser").innerHTML = "<a href=\"#\" onclick=\"mainxfer_send_redirect('XfeRLOCAL','" + lastcustchannel + "','" + lastcustserverip + "');return false;\"><IMG SRC=\"../agc/images/vdc_XB_localcloser.gif\" border=0 alt=\"ЛОКАЛЬНЫЙ CLOSER\" style=\"vertical-align:middle\"></a>";

								document.getElementById("DialBlindTransfer").innerHTML = "<a href=\"#\" onclick=\"mainxfer_send_redirect('XfeRBLIND','" + lastcustchannel + "','" + lastcustserverip + "');return false;\"><IMG SRC=\"../agc/images/vdc_XB_blindtransfer.gif\" border=0 alt=\"Слепой Перевод Набора номера\" style=\"vertical-align:middle\"></a>";
								...
								if (quick_transfer_button == 'IN_GROUP')
									{
									document.getElementById("QuickXfer").innerHTML = "<a href=\"#\" onclick=\"mainxfer_send_redirect('XfeRLOCAL','" + lastcustchannel + "','" + lastcustserverip + "');return false;\"><IMG SRC=\"../agc/images/vdc_LB_quickxfer.gif\" border=0 alt=\"Быстрой передачи\"></a>";
									}
								...
								if ( (quick_transfer_button == 'PRESET_1') || (quick_transfer_button == 'PRESET_2') || (quick_transfer_button == 'PRESET_3') || (quick_transfer_button == 'PRESET_4') || (quick_transfer_button == 'PRESET_5') )
									{
									document.getElementById("QuickXfer").innerHTML = "<a href=\"#\" onclick=\"mainxfer_send_redirect('XfeRBLIND','" + lastcustchannel + "','" + lastcustserverip + "');return false;\"><IMG SRC=\"../agc/images/vdc_LB_quickxfer.gif\" border=0 alt=\"Быстрой передачи\"></a>";
		...
		if (MD_ring_secondS > 49) 
			...
			MD_channel_look=0;
			MD_ring_secondS=0;
			alert("Превышено время ожидания набора номера, свяжитесь с сисадмином\n");
			}
				...
				//	alert(xmlhttp.responseText);
					...
					if (UDresponse_status == 'GOOD')
						...
						if (enable_second_webform > 0)
							{
							document.getElementById("WebFormSpanTwo").innerHTML = "<a href=\"" + TEMP_VDIC_web_form_address_two + "\" target=\"" + web_form_target + "\" onMouseOver=\"WebFormTwoRefresH();\"><IMG SRC=\"../agc/images/vdc_LB_webform_two.gif\" border=0 alt=\"Web Форма 2\"></a>\n";
							}
						}
					else
						{
...
// ################################################################################
// Send the Manual Набрать следующий номер request
	function ManualDialNext(mdnCBid,mdnBDleadid,mdnDiaLCodE,mdnPhonENumbeR,mdnStagE,mdVendorid,mdgroupalias)
		...
		if (dial_method == "INBOUND_MAN")
			...
			document.getElementById("DiaLControl").innerHTML = "<IMG SRC=\"../agc/images/vdc_LB_pause_OFF.gif\" border=0 alt=\" Пауза \"><IMG SRC=\"../agc/images/vdc_LB_resume_OFF.gif\" border=0 alt=\"Резюме\"><BR><IMG SRC=\"../agc/images/vdc_LB_dialnextnumber_OFF.gif\" border=0 alt=\"Набрать следующий номер\">";
			}
		else
			{
			document.getElementById("DiaLControl").innerHTML = "<IMG SRC=\"../agc/images/vdc_LB_dialnextnumber_OFF.gif\" border=0 alt=\"Набрать следующий номер\">";
			}
		if (document.vicidial_form.LeadPreview.checked==true)
			{
			reselect_preview_dial = 1;
			var man_preview = 'YES';
			var man_status = "Просмотр Lead тогда <a href=\"#\" onclick=\"ManualDialOnly()\"><font class=\"preview_text\"> НАБОР LEAD</font></a> or <a href=\"#\" onclick=\"ManualDialSkip()\"><font class=\"preview_text\">ПРОПУСТИТЬ LEAD</font></a>"; 
			}
		else
			{
			reselect_preview_dial = 0;
			var man_preview = 'NO';
			var man_status = "Ожидание Кольца вызова..."; 
			}
			...
			//		alert(xmlhttp.responseText);
					...
					if ( (MDnextCID.match(regMNCvar)) || (MDnextCID.match(regMDFvarDNC)) || (MDnextCID.match(regMDFvarCAMP)) )
						...
						if (MDnextCID.match(regMNCvar))
							{alert("Нет больше leads для кампании в контейнере:\n" + campaign);   alert_displayed=1;}
						...
						if (MDnextCID.match(regMDFvarCAMP))
							{alert("Этот телефон не находится в списках кампании:\n" + mdnPhonENumbeR);   alert_displayed=1;}
						if (alert_displayed==0)						
							{alert("Неизвестная ошибка:\n" + mdnPhonENumbeR + "|" + MDnextCID);   alert_displayed=1;}

						if (starting_dial_level == 0)
							{
							document.getElementById("DiaLControl").innerHTML = "<a href=\"#\" onclick=\"ManualDialNext('','','','','','0');\"><IMG SRC=\"../agc/images/vdc_LB_dialnextnumber.gif\" border=0 alt=\"Набрать следующий номер\"></a>";
							}
						else
							{
							if (dial_method == "INBOUND_MAN")
								...
								document.getElementById("DiaLControl").innerHTML = "<IMG SRC=\"../agc/images/vdc_LB_pause_OFF.gif\" border=0 alt=\" Пауза \"><a href=\"#\" onclick=\"AutoDial_ReSume_PauSe('VDADready');\"><IMG SRC=\"../agc/images/vdc_LB_resume.gif\" border=0 alt=\"Резюме\"></a><BR><a href=\"#\" onclick=\"ManualDialNext('','','','','','0');\"><IMG SRC=\"../agc/images/vdc_LB_dialnextnumber.gif\" border=0 alt=\"Набрать следующий номер\"></a>";
								}
							else
								{
					...
					//	CalL_ScripT_id									= MDnextResponse_array[37];
						...
						document.getElementById("MainStatuSSpan").innerHTML = " Запрос: " + status_display_number + " UID: " + MDnextCID + " &nbsp; " + man_status;
						...
						if (LeaDPreVDispO == 'CALLBK')
							{
							document.getElementById("CusTInfOSpaN").innerHTML = " <B> PREVIOUS ВЫЗОВBACK </B>";
							...
							document.getElementById("CBcommentsBoxC").innerHTML = "<b>Оператор:</b>" + CBuser;
							document.getElementById("CBcommentsBoxD").innerHTML = "<b>Комментарии:</b><br>" + CBcomments;
...
// Send the Manual Dial Skip
	function ManualDialSkip()
		...
		if (manual_dial_in_progress==1)
			{
			alert('ВЫ НЕ МОЖЕТЕ ПРОПУСТИТЬ ПОВТОРНЫЙ ЗВОНОК ИЛИ РУЧНОЙ НАБОР НОМЕРА, ВЫ ДОЛЖНЫ НАБОР НОМЕРА THE LEAD');
			}
		else
			{
			if (dial_method == "INBOUND_MAN")
				...
				xmlhttp.onreadystatechange = function() 
					...
					//	alert(xmlhttp.responseText);
						...
						if (MDSnextCID == "LEAD NOT REVERTED")
							{
							alert("Lead не вернулся, была ошибка:\n" + MDSnextResponse);
							}
						else
							{
...
// Send the Manual Dial Only - dial the previewed lead
	function ManualDialOnly(taskaltnum)
		...
		if (dialed_label == 'ALT')
			{document.getElementById("CusTInfOSpaN").innerHTML = " <B> ALT номер: ALT </B>";}
		if (dialed_label == 'ADDR3')
			{document.getElementById("CusTInfOSpaN").innerHTML = " <B> ALT номер: АДРЕСС 3 </B>";}
		var REGalt_dial = new RegExp("X","g");
		if (dialed_label.match(REGalt_dial))
			{
			document.getElementById("CusTInfOSpaN").innerHTML = " <B> ALT номер: " + dialed_label + "</B>";
			document.getElementById("EAcommentsBoxA").innerHTML = "<b>Телефон кодекса и номер: </b>" + EAphone_code + " " + EAphone_number;

			var EAactive_link = '';
			if (EAalt_phone_active == 'Y') 
				{EAactive_link = "<a href=\"#\" onclick=\"alt_phone_change('" + EAphone_number + "','" + EAalt_phone_count + "','" + document.vicidial_form.lead_id.value + "','N');\">Изменить этот телефонный номер НЕАКТИВНОЕ</a>";}
			else
				{EAactive_link = "<a href=\"#\" onclick=\"alt_phone_change('" + EAphone_number + "','" + EAalt_phone_count + "','" + document.vicidial_form.lead_id.value + "','Y');\">Изменить этот телефонный номер АКТИВНЫМ</a>";}

			document.getElementById("EAcommentsBoxB").innerHTML = "<b>Активно:</b>" + EAalt_phone_active + "<BR>" + EAactive_link;
			document.getElementById("EAcommentsBoxC").innerHTML = "<b>Alt Кол-во:</b>" + EAalt_phone_count;
		...
		if (xmlhttp) 
			...
			//		alert(xmlhttp.responseText);
					...
					if (MDnextCID == " ВЫЗОВ NOT PLACED")
						{
						alert("вызов не был обслужен, была ошибка:\n" + MDOnextResponse);
						}
					else
						{
						...
						if (alt_dial_status_display=='0')
							{
							document.getElementById("MainStatuSSpan").innerHTML = " Запрос: " + status_display_number + " UID: " + MDnextCID + " &nbsp; Ожидание Кольца вызова...";
	...
	function AutoDial_ReSume_PauSe(taskaction,taskagentlog,taskwrapup,taskstatuschange,temp_reason)
		...
		if (taskaction == 'VDADready')
			...
			if (dial_method == "INBOUND_MAN")
				...
				document.getElementById("DiaLControl").innerHTML = "<a href=\"#\" onclick=\"AutoDial_ReSume_PauSe('VDADpause');\"><IMG SRC=\"../agc/images/vdc_LB_pause.gif\" border=0 alt=\" Пауза \"></a><IMG SRC=\"../agc/images/vdc_LB_resume_OFF.gif\" border=0 alt=\"Резюме\"></a><BR><a href=\"#\" onclick=\"ManualDialNext('','','','','','0');\"><IMG SRC=\"../agc/images/vdc_LB_dialnextnumber.gif\" border=0 alt=\"Набрать следующий номер\"></a>";
				}
			else
				{
			...
			if ( (agent_pause_codes_active=='FORCE') && (temp_reason != 'ВЫХОД') && (temp_reason != 'REQUEUE') && (temp_reason != 'DIALNEXT') )
				{
				...
				xmlhttprequestcheckauto.onreadystatechange = function() 
					...
					//	alert(xmlhttprequestcheckauto.responseText);
						...
						//	DialLog("start");
							...
							document.getElementById("MainStatuSSpan").innerHTML = " Входящий: " + dial_display_number + " UID: " + CIDcheck + " &nbsp; " + VDIC_fronter; 
							...
							if (VDIC_data_VDIG[1].length > 0)
								...
								document.getElementById("MainStatuSSpan").innerHTML = " Входящий: " + dial_display_number + " Group- " + VDIC_data_VDIG[1] + " &nbsp; " + VDIC_fronter; 
								}
...
// refresh the content of the web form URL
	function WebFormRefresH(taskrefresh,submittask,force_webvars_refresh) 
		...
		if (taskrefresh == 'OUT')
			{
			document.getElementById("WebFormSpan").innerHTML = "<a href=\"" + TEMP_VDIC_web_form_address + "\" target=\"" + web_form_target + "\" onMouseOver=\"WebFormRefresH('IN');\"><IMG SRC=\"../agc/images/vdc_LB_webform.gif\" border=0 alt=\"Web Форма\"></a>\n";
			}
		else 
			{
			document.getElementById("WebFormSpan").innerHTML = "<a href=\"" + TEMP_VDIC_web_form_address + "\" target=\"" + web_form_target + "\" onMouseOut=\"WebFormRefresH('OUT');\"><IMG SRC=\"../agc/images/vdc_LB_webform.gif\" border=0 alt=\"Web Форма\"></a>\n";
			}
		}
...
// refresh the content of the second web form URL
	function WebFormTwoRefresH(taskrefresh,submittask) 
		...
		if (enable_second_webform > 0)
			...
			if (taskrefresh == 'OUT')
				{
				document.getElementById("WebFormSpanTwo").innerHTML = "<a href=\"" + TEMP_VDIC_web_form_address_two + "\" target=\"" + web_form_target + "\" onMouseOver=\"WebFormTwoRefresH('IN');\"><IMG SRC=\"../agc/images/vdc_LB_webform_two.gif\" border=0 alt=\"Web Форма 2\"></a>\n";
				}
			else 
				{
				document.getElementById("WebFormSpanTwo").innerHTML = "<a href=\"" + TEMP_VDIC_web_form_address_two + "\" target=\"" + web_form_target + "\" onMouseOut=\"WebFormTwoRefresH('OUT');\"><IMG SRC=\"../agc/images/vdc_LB_webform_two.gif\" border=0 alt=\"Web Форма 2\"></a>\n";
				}
			}
		}
...
// Send Hangup command for customer call connected to the conference now to Manager
	function dialedcall_send_hangup(dispowindow,hotkeysused,altdispo,nodeletevdac) 
		...
		//  DEACTIVATE CHANNEL-DEPENDANT BUTTONS AND VARIABLES
			...
			if( document.images ) { document.images['livecall'].src = image_livecall_OFF.src;}
			document.getElementById("WebFormSpan").innerHTML = "<IMG SRC=\"../agc/images/vdc_LB_webform_OFF.gif\" border=0 alt=\"Web Форма\">";
			if (enable_second_webform > 0)
				{
				document.getElementById("WebFormSpanTwo").innerHTML = "<IMG SRC=\"../agc/images/vdc_LB_webform_two_OFF.gif\" border=0 alt=\"Web Форма 2\">";
				}
			document.getElementById("ParkControl").innerHTML = "<IMG SRC=\"../agc/images/vdc_LB_parkcall_OFF.gif\" border=0 alt=\"Парковать Вызов\">";
			document.getElementById("HangupControl").innerHTML = "<IMG SRC=\"../agc/images/vdc_LB_hangupcustomer_OFF.gif\" border=0 alt=\"Разъединить Клиента\">";
			document.getElementById("XferControl").innerHTML = "<IMG SRC=\"../agc/images/vdc_LB_transferconf_OFF.gif\" border=0 alt=\"Перевод - Конференция\">";
			document.getElementById("LocalCloser").innerHTML = "<IMG SRC=\"../agc/images/vdc_XB_localcloser_OFF.gif\" border=0 alt=\"ЛОКАЛЬНЫЙ CLOSER\" style=\"vertical-align:middle\">";
			document.getElementById("DialBlindTransfer").innerHTML = "<IMG SRC=\"../agc/images/vdc_XB_blindtransfer_OFF.gif\" border=0 alt=\"Слепой Перевод Набора номера\" style=\"vertical-align:middle\">";
			...
			if (quick_transfer_button_enabled > 0)
				{document.getElementById("QuickXfer").innerHTML = "<IMG SRC=\"../agc/images/vdc_LB_quickxfer_OFF.gif\" border=0 alt=\"Быстрой передачи\">";}
...
// Send Hangup command for 3rd party call connected to the conference now to Manager
	function xfercall_send_hangup() 
		...
		//	document.getElementById("Leave3WayCall").innerHTML ="<IMG SRC=\"../agc/images/vdc_XB_leave3waycall_OFF.gif\" border=0 alt=\"ОСТАВИТЬ 3-WAY ЗАПРОС\">";

			document.getElementById("DialWithCustomer").innerHTML ="<a href=\"#\" onclick=\"SendManualDial('YES');return false;\"><IMG SRC=\"../agc/images/vdc_XB_dialwithcustomer.gif\" border=0 alt=\"Набор номера С Клиентом\" style=\"vertical-align:middle\"></a>";

			document.getElementById("ParkCustomerDial").innerHTML ="<a href=\"#\" onclick=\"xfer_park_dial();return false;\"><IMG SRC=\"../agc/images/vdc_XB_parkcustomerdial.gif\" border=0 alt=\"Парковать Набор номера Клиента\" style=\"vertical-align:middle\"></a>";

			document.getElementById("HangupXferLine").innerHTML ="<IMG SRC=\"../agc/images/vdc_XB_hangupxferline_OFF.gif\" border=0 alt=\"Разъединение Xfer Линии\" style=\"vertical-align:middle\">";
...
// Generate the Call Disposition Chooser panel
	function DispoSelectContent_create(taskDSgrp,taskDSstage)
		...
		var dispo_HTML = "<table cellpadding=5 cellspacing=5 width=500><tr><td colspan=2><B> ВЫЗОВ DISPOSITION</B></td></tr><tr><td bgcolor=\"#99FF99\" height=300 width=240 valign=top><font class=\"log_text\"><span id=DispoSelectA>";
		var loop_ct = 0;
...
// Generate the Пауза Code Chooser panel
	function PauseCodeSelectContent_create()
		...
		if ( (AutoDialWaiting == 1) || (VD_live_customer_call==1) || (alt_dial_active==1) )
			...
			PauseCode_HTML = "<table cellpadding=5 cellspacing=5 width=500><tr><td colspan=2><B> ПАУЗПА CODE</B></td></tr><tr><td bgcolor=\"#99FF99\" height=300 width=240 valign=top><font class=\"log_text\"><span id=PauseCodeSelectA>";
			var loop_ct = 0;
...
// ################################################################################
// Generate the Группа Alias Chooser panel
	function GroupAliasSelectContent_create(task3way)
		...
		if (task3way > 0)
			...
			GroupAlias_HTML = GroupAlias_HTML + "<font size=2 style=\"BACKGROUND-COLOR: #FFFFCC\"><b><a href=\"#\" onclick=\"GroupAliasSelect_submit('ПОЛЬЗОВАТЕЛЬСКИЙER','" + document.vicidial_form.phone_number.value + "','0');return false;\">ПОЛЬЗОВАТЕЛЬСКИЙER - " + document.vicidial_form.phone_number.value + "</a></b></font><BR><BR>";
...
// open web form, then submit disposition
	function WeBForMDispoSelect_submit()
		...
		if (DispoChoice.length < 1) {alert("Вы Должны Выбрать Диспозицию");}
		else
			{
	...
	function DispoSelect_submit()
		...
		if (DispoChoice.length < 1) {alert("Вы Должны Выбрать Диспозицию");}
			...
			if ( (DispoChoice == 'CALLBK') && (scheduled_callbacks > 0) ) {showDiv('CallBackSelectBox');}
				...
				document.getElementById("DispoSelectMaxMin").innerHTML = "<a href=\"#\" onclick=\"DispoMinimize()\"> минимизировать </a>";
				document.getElementById("DispoSelectHAspan").innerHTML = "<a href=\"#\" onclick=\"DispoHanguPAgaiN()\">Разъединить Снова</a>";
...
// Submit the Группа Alias 
	function GroupAliasSelect_submit(newgroupalias,newgroupcid,newusegroup)
		...
		if (newusegroup > 0)
			...
			document.getElementById("ManuaLDiaLGrouPSelecteD").innerHTML = "<font size=2 face=\"Arial,Helvetica\">Группа Alias: " + active_group_alias + "</font>";
			document.getElementById("XfeRDiaLGrouPSelecteD").innerHTML = "<font size=1 face=\"Arial,Helvetica\">Группа Alias: " + active_group_alias + "</font>";
			}
...
// Generate the Closer In Group Chooser panel
	function CloserSelectContent_create()
		...
		if ( (VU_agent_choose_ingroups == '1') && (manager_ingroups_set < 1) )
			{
			var live_CSC_HTML = "<table cellpadding=5 cellspacing=5 width=500><tr><td><B>ГРУППЫ НЕ ВЫБРАНЫ</B></td><td><B>ВЫБРАННЫЕ ГРУППЫ</B></td></tr><tr><td bgcolor=\"#99FF99\" height=300 width=240 valign=top><font class=\"log_text\"><span id=CloserSelectAdd> &nbsp; <a href=\"#\" onclick=\"CloserSelect_change('-----ADD-ALL-----','ADD');return false;\"><B>--- ADD ALL ---</B><BR>";
			var loop_ct = 0;
			...
			var live_CSC_HTML = "Менеджерhas selected groups for you<BR>";
			document.vicidial_form.CloserSelectList.value = '';
			document.getElementById("CloserSelectContent").innerHTML = live_CSC_HTML;
			}
		}
	...
	function TerritorySelectContent_create()
		...
		if (agent_select_territories == '1')
			...
			if (agent_choose_territories > 0)
				...
				var live_TERR_HTML = "Менеджерhas selected territories for you<BR>";
				document.vicidial_form.TerritorySelectList.value = '';
				document.getElementById("TerritorySelectContent").innerHTML = live_TERR_HTML;
				}
			}
	...
	function BrowserCloseLogout()
		...
		if (logout_stop_timeouts < 1)
			...
			if (VDRP_stage != 'PAUSED')
				{
				AutoDial_ReSume_PauSe("VDADpause",'','','',"ВЫХОД");
				}
			LogouT('CLOSE');
			alert("PLEASE CLICK THE ВЫХОД LINK TO LOG OUT NEXT TIME.\n");
			}
		}
...
// Log the user out of the system, if active call or active dial is occuring, don't let them.
	function LogouT(tempreason)
		...
		if (MD_channel_look==1)
			{alert("Вы не можете выйти из системы во время попытки Набора номера. \nЖдите 50 секунд набора номера для явного 'нет ответа'");}
		else
			{
			if (VD_live_customer_call==1)
				{
				alert("ВСЕ ЕЩЕ АКТИВНЫЙ ЗАПРОС! Разъедините и тогда выходите из системы.\n" + VD_live_customer_call);
				}
			else
				{
				var xmlhttp=false;
				...
				var logout_content='';
				if (tempreason=='SHIFT')
					{logout_content='Ваш Shift больше или изменилась, Вы вышли из Вашей сессии<BR><BR>';}

				document.getElementById("LogouTBoxLink").innerHTML = logout_content + "<a href=\"" + agcPAGE + "?relogin=YES&session_epoch=" + epoch_sec + "&session_id=" + session_id + "&session_name=" + session_name + "&VD_login=" + user + "&VD_campaign=" + campaign + "&phone_login=" + original_phone_login + "&phone_pass=" + phone_pass + "&VD_pass=" + pass + "\">КЛИКНИТЕ ЗДЕСЬ, ЧТОБЫ СНОВА ВОЙТИ</a>\n";
...
// ################################################################################
// Обновление the agents view sidebar or xfer frame
...
// ################################################################################
// Обновление the calls in queue bottombar
...
// Open or close the callsinqueue view bottombar
	function show_calls_in_queue(CQoperation)
		...
		if (CQoperation=='SHOW')
			{
			document.getElementById("callsinqueuelink").innerHTML = "<a href=\"#\"  onclick=\"show_calls_in_queue('HIDE');\">Скрыть Звонки в очередь</a>";
			view_calls_in_queue_active=1;
			showDiv('callsinqueuedisplay');
			}
		else
			{
			document.getElementById("callsinqueuelink").innerHTML = "<a href=\"#\"  onclick=\"show_calls_in_queue('SHOW');\">Показать Звонки в очередь</a>";
...
// ################################################################################
// Move the Dispo frame out of the way and change the link to максимизировать
	function DispoMinimize()
		...
		document.getElementById("DispoSelectMaxMin").innerHTML = "<a href=\"#\" onclick=\"DispoMaximize()\"> максимизировать </a>";
		}


// ################################################################################
// Move the Dispo frame to the top and change the link to минимизировать
...
// Показать the groups selection span
	function OpeNGrouPSelectioN()
		...
		if ( (AutoDialWaiting == 1) || (VD_live_customer_call==1) || (alt_dial_active==1) )
			{
			alert("ТЫ ДОЛЖЕН БЫТЬ ПАУЗПАD сменить группу");
			}
		else
			{
			if (manager_ingroups_set > 0)
				{
				alert("Менеджер" + external_igb_set_name + " выбрал в вашей группе выбора");
				}
			else
				{
				HidEGenDerPulldown();
...
// Finish the wrapup timer early
	function TimerActionRun()
		...
		if ( (timer_action_message.length > 0) || (timer_action == 'MESSAGE_ONLY') )
			{
			document.getElementById("TimerContentSpan").innerHTML = "<b>ТАЙМЕР УВЕДОМЛЕНИЯ: " + timer_action_seconds + "секунды<BR><BR>" + timer_action_message + "</b>";
...
// GLOBAL FUNCTIONS
	...
	function start_all_refresh()
		...
		//	if ( (agentcall_manual != '1') && (starting_dial_level > 0) )
			...
			if (callholdstatus != '1')
				{hideDiv('AgentСтатусCalls');}
			if (agentcallsstatus != '1')
				{hideDiv('AgentСтатусSpan');}
			...
			if ( (agent_pause_codes_active=='Y') || (agent_pause_codes_active=='FORCE') )
				{
				document.getElementById("PauseCodeLinkSpan").innerHTML = "<a href=\"#\" onclick=\"PauseCodeSelectContent_create();return false;\">ВВЕДИТЕ КОД ПАУЗЫ</a>";
				}
			...
			if ( (VtigeRLogiNScripT == 'Y') && (VtigeREnableD > 0) )
				{
				document.getElementById("ScriptContents").innerHTML = "<iframe src=\"" + VtigeRurl + "/index.php?module=Пользовательs&action=Authenticate&return_module=Пользовательs&return_action=Логин&user_name=" + user + "&user_password=" + pass + "&login_theme=softed&login_language=en_us\" style=\"background-color:transparent;\" scrolling=\"auto\" frameborder=\"0\" allowtransparency=\"true\" id=\"popupFrame\" name=\"popupFrame\" width=\"" + script_width + "\" height=\"" + script_height + "\" STYLE=\"z-index:17\"> </iframe> ";
				}
			if ( (VtigeRLogiNScripT == 'NEW_WINDOW') && (VtigeREnableD > 0) )
				{
				var VtigeRall = VtigeRurl + "/index.php?module=Пользовательs&action=Authenticate&return_module=Пользовательs&return_action=Логин&user_name=" + user + "&user_password=" + pass + "&login_theme=softed&login_language=en_us";
			...
			if (WaitingForNextStep==0)
				...
				if (agent_status_view_active > 0)
					{
					refresh_agents_view('AgentViewСтатус',agent_status_view);
					}
	...
	function clearDiv(divvar)
		...
		if (document.getElementById(divvar))
			...
			if (divvar == 'DiaLLeaDPrevieW')
				{
				var buildDivHTML = "<font class=\"preview_text\"> <input type=checkbox name=LeadPreview size=1 value=\"0\"> ПРЕДВАРИТЕЛЬНЫЙ ПРОСМОТР LEAD<BR></font>";
				document.getElementById("DiaLLeaDPrevieWHide").innerHTML = buildDivHTML;
				}
			if (divvar == 'DiaLDiaLAltPhonE')
				{
				var buildDivHTML = "<font class=\"preview_text\"> <input type=checkbox name=DiaLAltPhonE size=1 value=\"0\"> НАБРАТЬ АЛЬТ. НОМЕР<BR></font>";
	...
	function conf_channels_detail(divvar) 
		...
		if (divvar == 'SHOW')
			...
			document.getElementById("busycallsdisplay").innerHTML = "<a href=\"#\"  onclick=\"conf_channels_detail('HIDE');\">Скрыть информацию о канале конференц-связи</a>";
			LMAe[0]=''; LMAe[1]=''; LMAe[2]=''; LMAe[3]=''; LMAe[4]=''; LMAe[5]=''; 
			LMAcount=0;
			}
		else
			...
			document.getElementById("busycallsdisplay").innerHTML = "<a href=\"#\"  onclick=\"conf_channels_detail('SHOW');\">Показать информацию о канале конференц-связи</a><BR><BR>&nbsp;";
			document.getElementById("outboundcallsspan").innerHTML = '';
			LMAe[0]=''; LMAe[1]=''; LMAe[2]=''; LMAe[3]=''; LMAe[4]=''; LMAe[5]=''; 
			LMAcount=0;
			}
	...
	function HotKeys(HKstate) 
		...
		if ( (HKstate == 'ON') && (HKbutton_allowed == 1) )
			...
			showDiv('HotKeyEntriesBox');
			hot_keys_active = 1;
			document.getElementById("hotkeysdisplay").innerHTML = "<a href=\"#\" onMouseOut=\"HotKeys('OFF')\"><IMG SRC=\"../agc/images/vdc_XB_hotkeysactive.gif\" border=0 alt=\"АКТИВНЫЕ ГОРЯЧИЕ КЛАВИШИ\"></a>";
			}
		else
			{
			hideDiv('HotKeyEntriesBox');
			hot_keys_active = 0;
			document.getElementById("hotkeysdisplay").innerHTML = "<a href=\"#\" onMouseOver=\"HotKeys('ON')\"><IMG SRC=\"../agc/images/vdc_XB_hotkeysactive_OFF.gif\" border=0 alt=\"НЕАКТИВНЫЕ ГОРЯЧИЕ КЛАВИШИ\"></a>";
			}
		}
	...
	function ShoWTransferMain(showxfervar,showoffvar)
		...
		if (VU_vicidial_transfers == '1')
			...
			if (showxfervar == 'ON')
				...
				document.getElementById("XferControl").innerHTML = "<a href=\"#\" onclick=\"ShoWTransferMain('OFF','YES');\"><IMG SRC=\"../agc/images/vdc_LB_transferconf.gif\" border=0 alt=\"Перевод - Конференция\"></a>";
				if (quick_transfer_button_enabled > 0)
					{document.getElementById("QuickXfer").innerHTML = "<IMG SRC=\"../agc/images/vdc_LB_quickxfer_OFF.gif\" border=0 alt=\"Быстрой передачи\">";}
				}
			else
			...
			if (three_way_call_cid == 'AGENT_CHOOSE')
				...
				document.getElementById("XfeRDiaLGrouPSelecteD").innerHTML = "<font size=1 face=\"Arial,Helvetica\">Группа Alias: " + active_group_alias + "</font>";
				document.getElementById("XfeRCID").innerHTML = "<a href=\"#\" onclick=\"GroupAliasSelectContent_create('1');\"><font size=1 face=\"Arial,Helvetica\">Нажмите здесь, чтобы Выбери группу Alias</font></a>";
				}
			else
				{
			...
			if (showxfervar != 'OFF')
				{
				alert('У ВАС НЕТ РАЗРЕШЕНИЯ ДЛЯ ПЕРЕВОДА ВЫЗОВА');
				}
			}
		}
...
<?php	echo "Пользователь: $VD_login на Телефон: $SIP_user в кампанию: $VD_campaign&nbsp; \n"; ?>
...
<TD WIDTH=<?php echo $HSwidth ?> VALIGN=MIDDLE ALIGN=CENTER><font class="body_text">&nbsp; <span id=status>LIVE</span>&nbsp; &nbsp;session ID: <span id=sessionIDspan></span>&nbsp; &nbsp;<span id=AgentСтатусCalls></span></TD>
<TD WIDTH=109><IMG SRC="../agc/images/agc_live_call_OFF.gif" NAME=livecall ALT="Живой Вызов" WIDTH=109 HEIGHT=30 Border=0></TD>
</TR></TABLE>
</span>



<span style="position:absolute;left:0px;top:0px;z-index:3;" id="Добро пожаловатьBoxA">
    <table border=0 bgcolor="#FFFFFF" width=<?php echo $CAwidth ?> height=<?php echo $HKwidth ?>><TR><TD align=center><BR><span id="Добро пожаловатьBoxAt">Agent Screen</span></TD></TR></TABLE>
</span>

<span style="position:absolute;left:300px;top:<?php echo $MBheight ?>px;z-index:12;" id="ManuaLDiaLButtons"><font class="body_text">
<span id="MDstatusSpan"><a href="#" onclick="NeWManuaLDiaLCalL('NO');return false;">РУЧНОЙ НАБОР НОМЕРА</a></span> &nbsp; &nbsp; &nbsp; <a href="#" onclick="NeWManuaLDiaLCalL('FAST');return false;">БЫСТРЫЙ НАБОР НОМЕРА</a><BR>
</font></span>

<span style="position:absolute;left:300px;top:<?php echo $CBheight ?>px;z-index:13;" id="CallbacksButtons"><font class="body_text">
<span id="CBstatusSpan">X АКТИВНЫЕ ВЫЗОВBACK</span> <BR>
</font></span>
...
<span style="position:absolute;left:<?php echo $SCwidth ?>px;top:49px;z-index:18;" id="SecondSspan"><font class="body_text">секунды: 
...
<span style="position:absolute;left:<?php echo $AMwidth ?>px;top:<?php echo $AMheight ?>px;z-index:22;" id="AgentMuteANDPreseTDiaL"><font class="body_text">
	...
	if ($PreseT_DiaL_LinKs)
		{
		echo "<a href=\"#\" onclick=\"DtMf_PreSet_a_DiaL();return false;\"><font class=\"body_tiny\">D1 - НАБОР НОМЕРА</font></a>\n";
		echo "<BR>\n";
		echo "<a href=\"#\" onclick=\"DtMf_PreSet_b_DiaL();return false;\"><font class=\"body_tiny\">D2 - НАБОР НОМЕРА</font></a>\n";
		}
	else {echo "<BR>\n";}
	?>
<BR><BR> &nbsp; <BR>
...
<span style="position:absolute;left:0px;top:0px;z-index:51;" id="CallBacKsLisTBox">
	...
	<BR> &nbsp; 
	<a href="#" onclick="CalLBacKsLisTCheck();return false;">Обновление</a>
	 &nbsp;  &nbsp;  &nbsp;  &nbsp;  &nbsp;  &nbsp;  &nbsp;  &nbsp; 
	<a href="#" onclick="hideDiv('CallBacKsLisTBox');return false;">Вернуться</a>
	</TD></TR></TABLE>
</span>
...
<span style="position:absolute;left:0px;top:0px;z-index:52;" id="NeWManuaLDiaLBox">
	...
	if (eregi("X",dial_prefix))
		{
		echo "Примечание: префикс набора номера  $dial_prefix будет добавлен в начале этого номера<BR>\n";
		}
	?>
	Note: all new manual dial leads will go into list <?php echo $manual_dial_list_id ?><BR><BR>
	<table><tr>
	<td align=right><font class="body_text"> Код страны: </td>
	<td align=left><font class="body_text"><input type=text size=7 maxlength=10 name=MDDiaLCodE class="cust_form" value="1">&nbsp; (Обычно 7 для России и некоторых стран СНГ)</td>
	</tr><tr>
	<td align=right><font class="body_text"> Телефон Number: </td>
	...
	<td align=right><font class="body_text"> Поиск Существующих Leads: </td>
	<td align=left><font class="body_text"><input type=checkbox name=LeadLookuP size=1 value="0">&nbsp; (Если эта опция выбрана, то будет предпринята попытка найти телефонный номер в системе, прежде чем вставить его как новый lead)</td>
	</tr><tr>
	...
	<BR><BR>Если вы хотите набрать номер, но не добовлять новый lead, введите точный dialstring который вы хотите вызвать в поле Отмены Номера ниже. Для разъединения этого запроса вы должны будете открыть ссылку ЗАПРОСЫ В ЭТОЙ СЕССИИ внизу экрана, at the bottom of the screen and hang it up by clicking на its channel link there.<BR> &nbsp; </td>
	</tr><tr>
	<td align=right><font class="body_text"> Отмененный набор номера: </td>
	<td align=left><font class="body_text"><input type=text size=24 maxlength=20 name=MDDiaLOverridE class="cust_form" value="">&nbsp; (только цифры, пожалуйста)
	</td>
	</tr></table>
	<BR>
	<a href="#" onclick="NeWManuaLDiaLCalLSubmiT('NOW');return false;">Новый Набор номера</a>
	 &nbsp;  &nbsp;  &nbsp;  &nbsp;  &nbsp;  &nbsp;  &nbsp;  &nbsp; 
	<a href="#" onclick="NeWManuaLDiaLCalLSubmiT('PREVIEW');return false;">Предварительный Звоните</a>
	 &nbsp;  &nbsp;  &nbsp;  &nbsp;  &nbsp;  &nbsp;  &nbsp;  &nbsp; 
	<a href="#" onclick="hideDiv('NeWManuaLDiaLBox');return false;">Вернуться</a>
	</TD></TR></TABLE>
</span>
...
<span style="position:absolute;left:35px;top:<?php echo $CBheight ?>px;z-index:20;" id="AgentСтатусSpan"><font class="body_text">
Ваш Статус: <span id="AgentСтатусСтатус"></span> <BR>Calls Dialing: <span id="AgentСтатусDiaLs"></span> 
</font></span>
...
<span style="position:absolute;left:157px;top:<?php echo $HTheight ?>px;z-index:32;" id="TransferMain">
	...
	<IMG SRC="../agc/images/vdc_XB_header.gif" border=0 alt="Перевод - Конференция" style="vertical-align:middle"> &nbsp; &nbsp; &nbsp; &nbsp; <span id="XfeRDiaLGrouPSelecteD"></span> &nbsp; &nbsp; <span id="XfeRCID"></span><BR>

	<TABLE cellpadding=0 cellspacing=1 border=0>
	<TR>
	<TD ALIGN=LEFT COLSPAN=3>
	<span id="XfeRGrouPLisT"><select size=1 name=XfeRGrouP class="cust_form"><option>-- SELECT A GROUP TO SEND YOUR ВЫЗОВ TO --</option></select></span>
	 
	<span STYLE="background-color: <?php echo $MAIN_COLOR ?>" id="LocalCloser"><IMG SRC="../agc/images/vdc_XB_localcloser_OFF.gif" border=0 alt="ЛОКАЛЬНЫЙ CLOSER" style="vertical-align:middle"></span> &nbsp; &nbsp;
	</TD>
	<TD ALIGN=LEFT>
	<span STYLE="background-color: <?php echo $MAIN_COLOR ?>" id="HangupXferLine"><IMG SRC="../agc/images/vdc_XB_hangupxferline_OFF.gif" border=0 alt="Разъединение Xfer Линии" style="vertical-align:middle"></span>
	</TD>
	</TR>
	...
	<TD ALIGN=LEFT>
	<input type=checkbox name=consultativexfer size=1 value="0"><font class="body_tiny"> КОНСУЛЬТАТИВНЫЙ &nbsp;</font>
	</TD>
	<TD ALIGN=LEFT>
	<span STYLE="background-color: <?php echo $MAIN_COLOR ?>" id="HangupBothLines"><a href="#" onclick="bothcall_send_hangup();return false;"><IMG SRC="../agc/images/vdc_XB_hangupbothlines.gif" border=0 alt="Разъединить Обе Линии" style="vertical-align:middle"></a></span>
	</TD>
	</TR>

	<TR>
	<TD ALIGN=LEFT COLSPAN=2>
	<IMG SRC="../agc/images/vdc_XB_number.gif" border=0 alt="Номер для вызова" style="vertical-align:middle">
	&nbsp; 
	...
	<TD ALIGN=LEFT>
	<input type=checkbox name=xferoverride size=1 value="0"><font class="body_tiny"> НАБЕРИТЕ ОТМЕНУ</font>
	</TD>
	<TD ALIGN=LEFT>
	<span STYLE="background-color: <?php echo $MAIN_COLOR ?>" id="Leave3WayCall"><a href="#" onclick="leave_3way_call('FIRST');return false;"><IMG SRC="../agc/images/vdc_XB_leave3waycall.gif" border=0 alt="ОСТАВИТЬ 3-WAY ЗАПРОС" style="vertical-align:middle"></a></span> 
	</TD>
	</TR>

	<TR>
	<TD ALIGN=LEFT COLSPAN=4>
	<span STYLE="background-color: <?php echo $MAIN_COLOR ?>" id="DialBlindTransfer"><IMG SRC="../agc/images/vdc_XB_blindtransfer_OFF.gif" border=0 alt="Слепой Перевод Набора номера" style="vertical-align:middle"></span>
	&nbsp;
	<span STYLE="background-color: <?php echo $MAIN_COLOR ?>" id="DialWithCustomer"><a href="#" onclick="SendManualDial('YES');return false;"><IMG SRC="../agc/images/vdc_XB_dialwithcustomer.gif" border=0 alt="Набор номера С Клиентом" style="vertical-align:middle"></a></span> 
	&nbsp;
	<span STYLE="background-color: <?php echo $MAIN_COLOR ?>" id="ParkCustomerDial"><a href="#" onclick="xfer_park_dial();return false;"><IMG SRC="../agc/images/vdc_XB_parkcustomerdial.gif" border=0 alt="Парковать Набор номера Клиента" style="vertical-align:middle"></a></span> 
	&nbsp;
...
<span style="position:absolute;left:0px;top:<?php echo $CQheight ?>px;width:<?php echo $MNwidth ?>;overflow:scroll;z-index:23;background-color:<?php echo $SIDEBAR_COLOR ?>;" id="callsinqueuedisplay"><TABLE CELLPADDING=0 CELLSPACING=0 Border=0><TR><TD width=5 ROWSPAN=2>&nbsp;</TD><TD ALIGN=CENTER><font class="body_text">Звонки в очередь: &nbsp; </font></TD></TR><TR><TD ALIGN=CENTER><span id="callsinqueuelist">&nbsp;</span></TD></TR></TABLE></span>
...
if ($view_calls_in_queue > 0)
	...
	if ($view_calls_in_queue_launch > 0) 
		{echo "<a href=\"#\" onclick=\"show_calls_in_queue('HIDE');\">Скрыть Звонки в очередь</a>\n";}
	else 
		{echo "<a href=\"#\" onclick=\"show_calls_in_queue('SHOW');\">Показать Звонки в очередь</a>\n";}
	}
?>
</span></font>
...
Другие агенты Статус: &nbsp; </font></TD></TR><TR><TD ALIGN=CENTER><span id="AgentViewСтатус">&nbsp;</span></TD></TR></TABLE></span>
...
Доступные Агенты передачи: <span id="AgentXferViewSelect"></span></CENTER></font></span>
...
<span style="position:absolute;left:5px;top:<?php echo $HTheight ?>px;z-index:34;" id="HotKeyActionBox">
	...
	<TR bgcolor="#FFEEBB"><TD height=70><font class="sh_text"> Lead Диспозицию как: </font><BR><BR><CENTER>
...
<span style="position:absolute;left:5px;top:<?php echo $HTheight ?>px;z-index:35;" id="HotKeyEntriesBox">
    <table border=0 bgcolor="#FFDD99" width=<?php echo $HCwidth ?> height=70>
	<TR bgcolor="#FFEEBB"><TD width=200><font class="sh_text"> Горячие Клавиши Диспозиции: </font></td><td colspan=2>
	<font class="body_small">Когда активно, просто нажмите клавишу желаемой диспозиции для этого запроса. Тогда запрос будет автоматически разъединен и dispositioned:</font></td></tr><tr>
...
<span style="position:absolute;left:5px;top:<?php echo $HTheight ?>px;z-index:36;" id="CBcommentsBox">
	...
	<TR bgcolor="#FFFF66">
	<TD align=left><font class="sh_text"> Предыдущая Информация о Callback: </font></td>
...
<span style="position:absolute;left:5px;top:<?php echo $HTheight ?>px;z-index:37;" id="EAcommentsBox">
    <table border=0 bgcolor="#FFFFCC" width=<?php echo $HCwidth ?> height=70>
	<TR bgcolor="#FFFF66">
	<TD align=left><font class="sh_text"> Расширенная информация Alt Телефон:</font></td>
	<TD align=right><font class="sk_text"> <a href="#" onclick="EAcommentsBoxhide('YES');return false;"> минимизировать </a> </font></td>
	</tr><tr>
...
<span style="position:absolute;left:695px;top:<?php echo $HTheight ?>px;z-index:38;" id="EAcommentsMinBox">
	...
	<TR bgcolor="#FFFF66">
	<TD align=left><font class="sk_text"><a href="#" onclick="EAcommentsBoxshow();return false;"> максимизировать </a> <BR>Alt Телефон Info</font></td>
	</tr></TABLE>
</span>
...
<span style="position:absolute;left:0px;top:0px;z-index:50;" id="CallBackSelectBox">
	...
	if ($agentonly_callbacks)
		{echo "<input type=checkbox name=CallBackOnlyMe id=CallBackOnlyMe size=1 value=\"0\"> ТОЛЬКО МОИ ВЫЗОВBACK <BR>";}
	?>
	CB Комментарии: <input type=text name="CallBackCommenTsField" id="CallBackCommenTsField" size=50 maxlength=255><BR><BR>
...
<span style="position:absolute;left:0px;top:0px;z-index:53;" id="CloserSelectBox">
	...
	if ( ($outbound_autodial_active > 0) and ($disable_blended_checkbox < 1) and ($dial_method != 'INBOUND_MAN') )
		{
		?>
		<input type=checkbox name=CloserSelectBlended size=1 value="0"> BLENDED ВЫЗОВING(исходящие активированы) <BR>
		<?php
		}
	?>
...
<span style="position:absolute;left:0px;top:0px;z-index:55;" id="NothingBox">
	...
	<span id="DiaLLeaDPrevieWHide">Канал</span>
	<span id="DiaLDiaLAltPhonEHide">Канал</span>
	<?php
	if (!$agentonly_callbacks)
		{echo "<input type=checkbox name=CallBackOnlyMe size=1 value=\"0\"> ТОЛЬКО МОИ ВЫЗОВBACK <BR>";}
	if ( ($outbound_autodial_active < 1) or ($disable_blended_checkbox > 0) or ($dial_method == 'INBOUND_MAN') )
		{echo "<input type=checkbox name=CloserSelectBlended size=1 value=\"0\"> BLENDED ВЫЗОВING<BR>";}
	?>
</span>
...
<span style="position:absolute;left:<?php echo $HKwidth ?>px;top:<?php echo $HKheight ?>px;z-index:16;" id="hotkeysdisplay"><a href="#" onMouseOver="HotKeys('ON')"><IMG SRC="../agc/images/vdc_XB_hotkeysactive_OFF.gif" border=0 alt="НЕАКТИВНЫЕ ГОРЯЧИЕ КЛАВИШИ"></a></span>
<?php } ?>


<span style="position:absolute;left:0px;top:<?php echo $HKheight ?>px;z-index:15;" id="MaiNfooterspan">
<table BGCOLOR="<?php echo $MAIN_COLOR ?>" id="MaiNfooter" width=<?php echo $MNwidth ?>><tr height=32><td height=32><font face="Arial,Helvetica" size=1>Agent web-client version: <?php echo $version ?> &nbsp; &nbsp; СБОРКА: <?php echo $build ?> &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; Сервер: <?php echo $server_ip ?>  &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp;</font><BR>
<font class="body_small">
<span id="busycallsdisplay"><a href="#"  onclick="conf_channels_detail('SHOW');">Показать информацию о канале конференц-связи</a>
...
<TR><TD colspan=3><font class="body_text"> СТАТУС: <span id="MainStatuSSpan"></span></font></TD></TR>
<tr><td colspan=3><span id="busycallsdebug"></span></td></tr>
<tr><td width=150 align=left valign=top>
<font class="body_text"><center>
<span STYLE="background-color: #CCFFCC" id="DiaLControl"><a href="#" onclick="ManualDialNext('','','','','','0');"><IMG SRC="../agc/images/vdc_LB_dialnextnumber_OFF.gif" border=0 alt="Набрать следующий номер"></a></span><BR>
<span id="DiaLLeaDPrevieW"><font class="preview_text"> <input type=checkbox name=LeadPreview size=1 value="0"> ПРЕДВАРИТЕЛЬНЫЙ ПРОСМОТР LEAD<BR></font></span>
<span id="DiaLDiaLAltPhonE"><font class="preview_text"> <input type=checkbox name=DiaLAltPhonE size=1 value="0"> НАБРАТЬ АЛЬТ. НОМЕР<BR></font></span>

<!--
<?php
if ( ($manual_dial_preview) and ($auto_dial_level==0) )
	{echo "<font class=\"preview_text\"> <input type=checkbox name=LeadPreview size=1 value=\"0\"> ПРЕДВАРИТЕЛЬНЫЙ ПРОСМОТР LEAD<BR></font>";}
if ( ($alt_phone_dialing) and ($auto_dial_level==0) )
	{echo "<font class=\"preview_text\"> <input type=checkbox name=DiaLAltPhonE size=1 value=\"0\"> НАБРАТЬ АЛЬТ. НОМЕР<BR></font>";}
...
ID ЗАПИСИ: <font class="body_small"><span id="RecorDID"></span></font><BR>
<center>
<!-- <a href=\"#\" onclick=\"conf_send_recording('MonitorConf','" + head_conf + "','');return false;\">Record</a> -->
<span STYLE="background-color: <?php echo $MAIN_COLOR ?>" id="RecorDControl"><a href="#" onclick="conf_send_recording('MonitorConf',session_id,'');return false;"><IMG SRC="../agc/images/vdc_LB_startrecording.gif" border=0 alt="Начать Запись"></a></span><BR>
<span id="SpacerSpanA"><IMG SRC="../agc/images/blank.gif" width=145 height=16 border=0></span><BR>
<span STYLE="background-color: #FFFFFF" id="WebFormSpan"><IMG SRC="../agc/images/vdc_LB_webform_OFF.gif" border=0 alt="Web Форма"></span><BR>
<?php
if ($enable_second_webform > 0)
	{echo "<span STYLE=\"background-color: #FFFFFF\" id=\"WebFormSpanTwo\"><IMG SRC=\"../agc/images/vdc_LB_webform_two_OFF.gif\" border=0 alt=\"Web Форма 2\"></span><BR>\n";}
?>
<span id="SpacerSpanB"><IMG SRC="../agc/images/blank.gif" width=145 height=16 border=0></span><BR>
<span STYLE="background-color: <?php echo $MAIN_COLOR ?>" id="ParkControl"><IMG SRC="../agc/images/vdc_LB_parkcall_OFF.gif" border=0 alt="Парковать Вызов"></span><BR>
<span STYLE="background-color: <?php echo $MAIN_COLOR ?>" id="XferControl"><IMG SRC="../agc/images/vdc_LB_transferconf_OFF.gif" border=0 alt="Перевод - Конференция"></span><BR>
...
<span STYLE="background-color: #FFCCFF" id="HangupControl"><IMG SRC="../agc/images/vdc_LB_hangupcustomer_OFF.gif" border=0 alt="Разъединить Клиента"></span><BR>
<span id="SpacerSpanD"><IMG SRC="../agc/images/blank.gif" width=145 height=16 border=0></span><BR>
<div class="text_input" id="SendDTMFdiv"><span STYLE="background-color: <?php echo $MAIN_COLOR ?>" id="SendDTMF"><a href="#" onclick="SendConfDTMF(session_id);return false;"><IMG SRC="../agc/images/vdc_LB_senddtmf.gif" border=0 alt="Послать DTMF" align=bottom></a>  <input type=text size=5 name=conf_dtmf class="cust_form" value="" maxlength=50></div></span><BR>
</center>
</font>
</td>
...
<table><tr>
<td align=right></td>
<td align=left><font class="body_text">&nbsp; Customer Time: <input type=text size=27 name=custdatetime class="cust_form" value=""> &nbsp; Канал: <input type=text size=27 name=callchannel class="cust_form" value=""></td>
</tr><tr>
<td colspan=2 align=center> Информация Клиента: <span id="CusTInfOSpaN"></span></td>
</tr><tr>
<td align=left colspan=2>
...
<td align=right><font class="body_text"> Телефон: </td>
<td align=left><font class="body_text">
<?php 
...
<td align=right valign=top><font class="body_text"> Комментарии:</td>
