...
if ($ACTION == 'regTERRITORY')
	...
	if ( (strlen($agent_territories)<1) || (strlen($user)<1) )
		{
		$channel_live=0;
		echo "Territory Choice $agent_territories это неправильно\n";
		exit;
		}
	else
		{
	...
	echo "Territory Choice $agent_territories был зарегистрован пользователем $user\n";
	}
		...
		if (strlen($phone_number)>3)
			...
			#	$DB=1;
				if (strlen($dial_statuses)>2)
					...
					if (eregi("UP LAST ВЫЗОВ TIME",$lead_order)){$order_stmt = 'order by last_local_call_time desc, lead_id asc';}
					if (eregi("DOWN LAST ВЫЗОВ TIME",$lead_order)){$order_stmt = 'order by last_local_call_time, lead_id asc';}
	...
	if ( (strlen($status)<1) || (strlen($agent_log_id)<1) )
		...
		if ($affected_rows > 0) 
			...
			if ($enable_queuemetrics_logging > 0)
				...
				$stmt = "INSERT INTO queue_log SET partition='P01',time_id='$StarTtime',call_id='NONE',queue='NONE',agent='Agent/$user',verb='ПАУЗПАREASON',serverid='$queuemetrics_log_id',data1='$status';";
				if ($DB) {echo "$stmt\n";}
				$rslt=mysql_query($stmt, $linkB);
...
if ($ACTION == 'AGENTSview')
	...
	echo "</TABLE><BR>\n";
	echo "<font style=\"font-size:10px;font-family:sans-serif;\"><font style=\"background-color:#ADD8E6;\"> &nbsp; &nbsp;</font>-READY &nbsp; <font style=\"background-color:#D8BFD8;\">&nbsp; &nbsp;</font>-INCALL &nbsp; <font style=\"background-color:#F0E68C;\"> &nbsp; &nbsp;</font>-ПАУЗПАD &nbsp;\n";
	if ($comments=='AgentXferViewSelect') 
		{
...
if ($ACTION == 'CALLSINQUEUEview')
	...
	if (eregi('NONE',$view_calls_in_queue))
		{
		echo "Звонки в Очередь Открыть инвалидов для этой кампании\n";
		exit;
		}
	else
		{
		...
		echo "<TD BGCOLOR=\"#CCCCCC\"><font style=\"font-size:11px;font-family:sans-serif;\"><B> &nbsp; ВЫЗОВ GROUP &nbsp; </font></TD>";
...
if ($ACTION == 'CALLSINQUEUEgrab')
	...
	if ( (eregi('NONE',$view_calls_in_queue)) or (eregi('N',$grab_calls_in_queue)) )
		{
		echo "ERROR: Звонки в Очередь Открыть инвалидов для этой кампании\n";
		exit;
		}
	else
		{
...
echo "\n<!-- время выполнения скрипта: $RUNtimeсекунды -->";
echo "\n</body>\n</html>\n";
}
