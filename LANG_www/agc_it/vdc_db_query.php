		...
		if (strlen($phone_number)>3)
			...
			#	$DB=1;
				if (strlen($dial_statuses)>2)
					...
					if (eregi("UP LAST CHIAMATA TIME",$lead_order)){$order_stmt = 'order by last_local_call_time desc, lead_id asc';}
					if (eregi("DOWN LAST CHIAMATA TIME",$lead_order)){$order_stmt = 'order by last_local_call_time, lead_id asc';}
...
if ($ACTION == 'AGENTSview')
	...
	while ($agents_count > $loop_count)
		...
		if ($comments=='AgentXferViewSelect') 
			{
			$AXVSuserORDINE[$j] =	"$full_name$US$j";
			$AXVSuser[$j] =			$user;
	...
	if ($comments=='AgentXferViewSelect') 
		...
		while ($j > $k)
			{
			$order_split = explode("_",$AXVSuserORDINE[$k]);
			$i = $order_split[1];
...
if ($ACTION == 'CALLSINQUEUEview')
	...
	if (eregi('NONE',$view_calls_in_queue))
		...
		echo "<TD BGCOLOR=\"#CCCCCC\"><font style=\"font-size:11px;font-family:sans-serif;\"><B> &nbsp; CHIAMATA GROUP &nbsp; </font></TD>";
...
if ($ACTION == 'CALLLOGview')
	...
	if ($g > 0)
		...
		{echo "<tr bgcolor=white><td colspan=11 align=center>No calls su this day</td></tr>";}
