		...
		if (strlen($phone_number)>3)
			...
			#	$DB=1;
				if (strlen($dial_statuses)>2)
					...
					if (eregi("UP LAST HOVOR TIME",$lead_order)){$order_stmt = 'order by last_local_call_time desc, lead_id asc';}
					if (eregi("DOWN LAST HOVOR TIME",$lead_order)){$order_stmt = 'order by last_local_call_time, lead_id asc';}
...
if ($ACTION == 'CALLSINQUEUEview')
	...
	if (eregi('NONE',$view_calls_in_queue))
		...
		echo "<TD BGCOLOR=\"#CCCCCC\"><font style=\"font-size:11px;font-family:sans-serif;\"><B> &nbsp; HOVOR GROUP &nbsp; </font></TD>";
