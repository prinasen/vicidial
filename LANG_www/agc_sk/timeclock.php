...
if ( ($stage == 'login') or ($stage == 'logout') )
	...
	if ($valid_user < 1)
		...
		if ($vts_count > 0)
			...
			$stmt="INSERT INTO vicidial_timeclock_status set status='SPUSTIŤ', user='$user', user_group='$user_group', event_epoch='$StarTtimE', ip_address='$ip';";
			if ($DB) {echo "$stmt\n";}
			$rslt=mysql_query($stmt, $link);
				$status='SPUSTIŤ';
				$totTIME_HMS='0:00:00';
		...
		if ( ($last_action_sec < 30) and ($status != 'SPUSTIŤ') )
			{
			### You cannot log in or out within 30sekúnd of your last login/logout
		...
		if ($commit == 'YES')
			{
			if ( ( ($status=='AUTOODHLÁSIŤ') or ($status=='SPUSTIŤ') or ($status=='ODHLÁSIŤ') ) and ($stage=='login') )
				{
			...
			if ( ($status=='LOGIN') and ($stage=='logout') )
				...
				### Add a record to the timeclock log
				$stmt="INSERT INTO vicidial_timeclock_log set event='ODHLÁSIŤ', user='$user', user_group='$user_group', event_epoch='$StarTtimE', ip_address='$ip', login_sec='$last_action_sec', event_date='$NOW_TIME';";
				if ($DB) {echo "$stmt\n";}
				$rslt=mysql_query($stmt, $link);
				$affected_rows = mysql_affected_rows($link);
				$timeclock_id = mysql_insert_id($link);
				...
				### Update the user's timeclock status record
				$stmt="UPDATE vicidial_timeclock_status set status='ODHLÁSIŤ', user_group='$user_group', event_epoch='$StarTtimE', ip_address='$ip' where user='$user';";
				if ($DB) {echo "$stmt\n";}
				$rslt=mysql_query($stmt, $link);
				$affected_rows = mysql_affected_rows($link);
				print "<!-- vicidial_timeclock_status record updated for $user:   |$affected_rows| -->\n";
				...
				### Add a record to the timeclock audit log
				$stmt="INSERT INTO vicidial_timeclock_audit_log set timeclock_id='$timeclock_id', event='ODHLÁSIŤ', user='$user', user_group='$user_group', event_epoch='$StarTtimE', ip_address='$ip', login_sec='$last_action_sec', event_date='$NOW_TIME';";
				if ($DB) {echo "$stmt\n";}
				$rslt=mysql_query($stmt, $link);
				$affected_rows = mysql_affected_rows($link);
				print "<!-- NOVÁ vicidial_timeclock_audit_log record inserted for $user:   |$affected_rows| -->\n";
			...
			if ( ( ( ($status=='AUTOODHLÁSIŤ') or ($status=='SPUSTIŤ') or ($status=='ODHLÁSIŤ') ) and ($stage=='logout') ) or ( ($status=='LOGIN') and ($stage=='login') ) )
			...
			if ($referrer=='admin') 
				{$BACKlink = "<A HREF=\"../vicidial/admin.php\"><font color=\"#003333\">SPÄŤ na administratívne</font></A>";}
			if ($referrer=='welcome') 
		...
		if ( ($status=='AUTOODHLÁSIŤ') or ($status=='SPUSTIŤ') or ($status=='ODHLÁSIŤ') )
			{
