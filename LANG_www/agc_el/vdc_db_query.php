		...
		if (strlen($phone_number)>3)
			...
			#	$DB=1;
				if (strlen($dial_statuses)>2)
					...
					if (eregi("UP LAST ΚΛΗΣΗ TIME",$lead_order)){$order_stmt = 'order by last_local_call_time desc, lead_id asc';}
					if (eregi("DOWN LAST ΚΛΗΣΗ TIME",$lead_order)){$order_stmt = 'order by last_local_call_time, lead_id asc';}
...
if ($ACTION == 'updateDISPO')
	...
	echo 'Lead ' . $lead_id . ' έχει αλλαχτεί ' . $dispo_choice . " Κατάσταση\nNext agent_log_id:\n" . $agent_log_id . "\n";
	}
...
if ($ACTION == 'CALLSINQUEUEview')
	...
	if (eregi('NONE',$view_calls_in_queue))
		...
		echo "<TD BGCOLOR=\"#CCCCCC\"><font style=\"font-size:11px;font-family:sans-serif;\"><B> &nbsp; ΚΛΗΣΗ GROUP &nbsp; </font></TD>";
