...
if (strlen($group[0]) < 1)
	...
	if (eregi("YES",$include_rollover))
		...
		if ( ($DROPcalls < 1) or ($CIcallsRAW < 1) )
			{$inDROPREPONSEpercent = '0';}
		else
			{
			$inDROPREPONSEpercent = (($DROPcallsRAW / $CIcallsRAW) * 100);
	...
	$OUToutput .= "\n";
	$OUToutput .= "---------- APPELS AUTOMATIQUES SANS REPONSES\n";
	...
	$OUToutput .= "\n";
	$OUToutput .= "---------- FAIT APPEL Hangup STATS\n";
	$OUToutput .= "+----------------------+------------+\n";
	...
	$OUToutput .= "| STATUS | DESCRIPTION          | CATÉGORIE             | CALLS      | TOTAL TIME | AVG TIME |CALLS/HOUR|CALLS/HOUR|\n";
	$OUToutput .= "+--------+----------------------+----------------------+------------+------------+----------+----------+----------+\n";
...
#	$OUToutput .= "|   AGENT TIME                                                      | $aTOTALhours |                     | $aTOTALrate |\n";
	...
	$OUToutput .= "\n";
	$OUToutput .= "---------- ID DE LA LISTE STATS\n";
	$OUToutput .= "+------------------------------------------+------------+\n";
	...
	$OUToutput .= "\n";
	$OUToutput .= "---------- CUSTOM STATUS CATÉGORIE STATS\n";
	$OUToutput .= "+----------------------+------------+--------------------------------+\n";
	...
	if ($costformat > 0)
		...
		echo "SORTANT $query_date to $end_date, &nbsp; $rawTOTtalk_min minutes at \$$outbound_rate = \$$outbound_cost</B>\n";
