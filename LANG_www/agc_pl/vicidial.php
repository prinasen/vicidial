			...
			if (xmlhttp) 
				...
				xmlhttp.onreadystatechange = function() 
					...
					//	alert(xmlhttp.responseText);
						...
						document.getElementById("LogiNReseT").innerHTML = "<INPUT TYPE=BUTTON VALUE=\"Odswież Kampania List\" OnClick=\"login_allowable_campaigns()\">";
...
echo "<span id=\"LogiNReseT\"><INPUT TYPE=BUTTON VALUE=\"Odswież Kampania List\" OnClick=\"login_allowable_campaigns()\"></span></TD></TR>\n";
	...
	if($auth>0)
		...
		if ( ( (ereg("START|ALL",$shift_enforcement)) and (!ereg("OFF",$VU_agent_shift_enforcement_override)) ) or (ereg("START|ALL",$VU_agent_shift_enforcement_override)) )
			...
			if ( ($shift_ok < 1) and ($VU_shift_override_flag < 1) and ($VDloginDISPLAY > 0) )
				...
				$VDdisplayMESSAGE.= "<INPUT TYPE=Submit NAME=ZATWIERDŹ VALUE=ZATWIERDŹ></FORM>\n";
				}
			}
...
### logged in and use that phone login account
	...
	if ( ( ($campaign_allow_inbound == 'Y') and ($dial_method != 'MANUAL') ) || ($campaign_leads_to_call > 0) || (ereg('Y',$no_hopper_leads_logins)) )
		...
		echo "<!-- stara Kolejka i Aktulanie wybierane numery powróciły na listę:   |$affected_rows| -->\n";
		...
		$affected_rows = mysql_affected_rows($link);
		echo "<!-- stara Kolejka i Aktualnie wybrane numery powróciły do zasobnika: |$affected_rows| -->\n";
		...
		$affected_rows = mysql_affected_rows($link);
		echo "<!-- stare rekordy vicidial_live_agents zostały wyczyszczone: |$affected_rows| -->\n";
...
$Cmonths = Array('Styczeń','February','Marzec','Kwiecień','Maj','Czerwiec',
				...
				xmlhttprequestcheckconf.onreadystatechange = function() 
					...
					//	alert(xmlhttprequestcheckconf.responseText);
						...
						if (InGroupChange > 0)
							...
							var external_igb_set_user = InGroupChangeUżytkownik;
...
// ZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZ  above section working na API functions
						...
						if ( (conf_channels_xtra_display == 1) || (conf_channels_xtra_display == 0) )
							...
							if (live_conf_calls > 0)
								...
								agentphonelive=0;
								var conv_start=-1;
								var live_conf_HTML = "<font face=\"Arial,Helvetica\"><B>AKTYWNE ROZMOWY W TWOJEJ SESJI:</B></font><BR><TABLE WIDTH=<?php echo $CQwidth ?>><TR BGCOLOR=<?php echo $SCRIPT_COLOR ?>><TD><font class=\"log_title\">#</TD><TD><font class=\"log_title\">KANAŁ ZDALNY</TD><TD><font class=\"log_title\">ROZŁĄCZ</TD><TD><font class=\"log_title\">VOLUME</TD></TR>";
		...
		if (xmlhttpXF) 
			...
			if ( (taskvar == 'XfeRBLIND') || (taskvar == 'XfeRVMAIL') )
				...
				if (blindxferdialstring.length<'2')
					...
					xferredirect_query='';
					taskvar = 'NOTHING';
					alert("Numer przekierowania musi mieć więcej niż 1 cyfrę:" + blindxferdialstring);
					}
				else
					{
			...
			if (xmlhttp) 
				...
				xmlhttp.onreadystatechange = function() 
					...
					//	alert(xmlhttp.responseText);
						...
						var loop_ct=0;
						var conv_start=0;
						var CB_HTML = "<table width=610><tr bgcolor=<?php echo $SCRIPT_COLOR ?>><td><font class=\"log_title\">#</td><td><font class=\"log_title\"> RozmowaBACK DATE/TIME</td><td><font class=\"log_title\">NUMER</td><td><font class=\"log_title\">NAME</td><td><font class=\"log_title\">  STATUS</td><td align=right><font class=\"log_title\">CAMPAIGN</td><td><font class=\"log_title\">LAST POŁĄCZENIE DATA/CZAS</td><td align=left><font class=\"log_title\"> COMMENTS</td></tr>"
...
// Fast version of manual dial
		...
		if ( (MDDiaLCodEform.length < 1) || (MDPhonENumbeRform.length < 5) )
			{
			alert("WY MUSICIE WCHODZIĆ NUMER TELEFONU I TARCZA SZYFROWAĆ UŻYWAĆ SZYBKI TARCZA");
			}
		else
			{
			var MDLookuPLeaD = 'new';
...
// Request lookup of manual dial channel
	function ManualDialCheckChanneL(taskCheckOR)
					...
					//		alert("channel not found yet:\n" + campaign);
						...
						if (taskCheckOR == 'YES')
							...
							if ( (XDchannel.match(regMDL)) && (asterisk_version != '1.0.8') && (asterisk_version != '1.0.9') && (MD_ring_secondS < 10) )
								...
								document.getElementById("MainStatuSSpan").innerHTML = " Zadzwoniono do trzeciej strony: " + document.vicidial_form.xfernumber.value + " UID: " + CIDcheck;
							...
							if ( (MDchannel.match(regMDL)) && (asterisk_version != '1.0.8') && (asterisk_version != '1.0.9') )
								...
								document.getElementById("MainStatuSSpan").innerHTML = " Zadzwoniono: " + status_display_number + " UID: " + CIDcheck + " &nbsp;"; 
		...
		if (MD_ring_secondS > 49) 
			...
			MD_channel_look=0;
			MD_ring_secondS=0;
			alert("Minął czas wybierania, skontaktuj się z administratorem\n");
			}
...
// Send the Manual Wybierz kolejny numer request
	function ManualDialNext(mdnCBid,mdnBDleadid,mdnDiaLCodE,mdnPhonENumbeR,mdnStagE,mdVendorid,mdgroupalias)
		...
		if (document.vicidial_form.LeadPreview.checked==true)
			...
			var man_preview = 'YES';
			var man_status = "Podejżyj dane połączenia i <a href=\"#\" onclick=\"ManualDialOnly()\"><font class=\"preview_text\"> WYBIERZ POŁĄCZENIE</font></a> or <a href=\"#\" onclick=\"ManualDialSkip()\"><font class=\"preview_text\">POMIŃ POŁĄCZENIE</font></a>"; 
			}
		else
			{
			...
			//		alert(xmlhttp.responseText);
					...
					if ( (MDnextCID.match(regMNCvar)) || (MDnextCID.match(regMDFvarDNC)) || (MDnextCID.match(regMDFvarCAMP)) )
						...
						if (MDnextCID.match(regMDFvarCAMP))
							{alert("Ten telefon nie jest w kampanii list:\n" + mdnPhonENumbeR);   alert_displayed=1;}
						if (alert_displayed==0)						
							{alert("Nieokreślony błąd:\n" + mdnPhonENumbeR + "|" + MDnextCID);   alert_displayed=1;}
					...
					//	CalL_ScripT_id									= MDnextResponse_array[37];
						...
						document.getElementById("MainStatuSSpan").innerHTML = " Dzwoni: " + status_display_number + " UID: " + MDnextCID + " &nbsp; " + man_status;
...
// Send the Manual Dial Skip
	function ManualDialSkip()
		...
		if (manual_dial_in_progress==1)
			{
			alert('NIE MOŻESZ POMINĄĆ RĘCZNEGO DZWONIENIA LUB ODDZWANIANIA, MUSISZ ZADZWONIĆ POD KONTAKT');
			}
		else
			{
			if (dial_method == "INBOUND_MAN")
				...
				xmlhttp.onreadystatechange = function() 
					...
					//	alert(xmlhttp.responseText);
						...
						if (MDSnextCID == "LEAD NOT REVERTED")
							{
							alert("Dane połączenia nie zostały przywrócowne, wystąpił błąd:\n" + MDSnextResponse);
							}
						else
							{
...
// Send the Manual Dial Only - dial the previewed lead
	function ManualDialOnly(taskaltnum)
		...
		if (dialed_label == 'ALT')
			{document.getElementById("CusTInfOSpaN").innerHTML = " <B> ALT WYBIERZ NUMER: ALT </B>";}
		if (dialed_label == 'ADDR3')
			{document.getElementById("CusTInfOSpaN").innerHTML = " <B> ALT WYBIERZ NUMER: ADRES3 </B>";}
		var REGalt_dial = new RegExp("X","g");
		if (dialed_label.match(REGalt_dial))
			{
			document.getElementById("CusTInfOSpaN").innerHTML = " <B> ALT WYBIERZ NUMER: " + dialed_label + "</B>";
		...
		if (xmlhttp) 
			...
			//		alert(xmlhttp.responseText);
					...
					if (MDnextCID == " Rozmowa NOT PLACED")
						{
						alert("nie wykonano połączenia, wystąpił błąd:\n" + MDOnextResponse);
						}
					else
						{
					...
					//	alert(xmlhttprequestcheckauto.responseText);
						...
						//	DialLog("start");
							...
							if (VDIC_data_VDIG[1].length > 0)
								...
								document.getElementById("MainStatuSSpan").innerHTML = " Przychodząca: " + dial_display_number + " Group- " + VDIC_data_VDIG[1] + " &nbsp; " + VDIC_fronter; 
								}
...
// Generate the Call Disposition Chooser panel
	function DispoSelectContent_create(taskDSgrp,taskDSstage)
		...
		var dispo_HTML = "<table cellpadding=5 cellspacing=5 width=500><tr><td colspan=2><B> Rozmowa DISPOSITION</B></td></tr><tr><td bgcolor=\"#99FF99\" height=300 width=240 valign=top><font class=\"log_text\"><span id=DispoSelectA>";
		var loop_ct = 0;
...
// Generate the Closer In Group Chooser panel
	function CloserSelectContent_create()
		...
		if ( (VU_agent_choose_ingroups == '1') && (manager_ingroups_set < 1) )
			{
			var live_CSC_HTML = "<table cellpadding=5 cellspacing=5 width=500><tr><td><B>GRUPY NIE WYBRANE</B></td><td><B>WYBRANE GRUPY</B></td></tr><tr><td bgcolor=\"#99FF99\" height=300 width=240 valign=top><font class=\"log_text\"><span id=CloserSelectAdd> &nbsp; <a href=\"#\" onclick=\"CloserSelect_change('-----ADD-ALL-----','ADD');return false;\"><B>--- ADD ALL ---</B><BR>";
			var loop_ct = 0;
	...
	function OpeNGrouPSelectioN()
		...
		if ( (AutoDialWaiting == 1) || (VD_live_customer_call==1) || (alt_dial_active==1) )
			...
			if (manager_ingroups_set > 0)
				{
				alert("Kierownik" + external_igb_set_name + " wybrał się w grupie-wybory");
				}
			else
				{
				HidEGenDerPulldown();
...
// GLOBAL FUNCTIONS
	...
	function start_all_refresh()
		...
		//	if ( (agentcall_manual != '1') && (starting_dial_level > 0) )
			...
			if ( (VtigeRLogiNScripT == 'Y') && (VtigeREnableD > 0) )
				{
				document.getElementById("ScriptContents").innerHTML = "<iframe src=\"" + VtigeRurl + "/index.php?module=Użytkowniks&action=Authenticate&return_module=Użytkowniks&return_action=Login&user_name=" + user + "&user_password=" + pass + "&login_theme=softed&login_language=en_us\" style=\"background-color:transparent;\" scrolling=\"auto\" frameborder=\"0\" allowtransparency=\"true\" id=\"popupFrame\" name=\"popupFrame\" width=\"" + script_width + "\" height=\"" + script_height + "\" STYLE=\"z-index:17\"> </iframe> ";
				}
			if ( (VtigeRLogiNScripT == 'NEW_WINDOW') && (VtigeREnableD > 0) )
				{
				var VtigeRall = VtigeRurl + "/index.php?module=Użytkowniks&action=Authenticate&return_module=Użytkowniks&return_action=Login&user_name=" + user + "&user_password=" + pass + "&login_theme=softed&login_language=en_us";
	...
	function clearDiv(divvar)
		...
		if (document.getElementById(divvar))
			...
			if (divvar == 'DiaLDiaLAltPhonE')
				{
				var buildDivHTML = "<font class=\"preview_text\"> <input type=checkbox name=DiaLAltPhonE size=1 value=\"0\"> WYBIERANIE ALT NUM<BR></font>";
	...
	function ShoWTransferMain(showxfervar,showoffvar)
		...
		if (VU_vicidial_transfers == '1')
			...
			if (showxfervar != 'OFF')
				{
				alert('NIE MASZ UPRAWNIEŃ DO PRZENOSZENIA ROZMÓW');
				}
			}
		}
...
<?php	echo "Zalogowany jako: $VD_login na Telefon: $SIP_user do kampani: $VD_campaign&nbsp; \n"; ?>
...
<span style="position:absolute;left:0px;top:0px;z-index:3;" id="WitajBoxA">
    <table border=0 bgcolor="#FFFFFF" width=<?php echo $CAwidth ?> height=<?php echo $HKwidth ?>><TR><TD align=center><BR><span id="WitajBoxAt">Agent Screen</span></TD></TR></TABLE>
</span>
...
<span style="position:absolute;left:0px;top:0px;z-index:51;" id="CallBacKsLisTBox">
	...
	<a href="#" onclick="CalLBacKsLisTCheck();return false;">Odswież</a>
	 &nbsp;  &nbsp;  &nbsp;  &nbsp;  &nbsp;  &nbsp;  &nbsp;  &nbsp; 
	<a href="#" onclick="hideDiv('CallBacKsLisTBox');return false;">Wróć</a>
	</TD></TR></TABLE>
</span>
...
<span style="position:absolute;left:0px;top:0px;z-index:52;" id="NeWManuaLDiaLBox">
	...
	if (eregi("X",dial_prefix))
		{
		echo "Komentarz: Prefiks numeru $dial_prefix będzie dodane na początku tego numeru<BR>\n";
		}
	?>
	Note: all new manual dial leads will go into list <?php echo $manual_dial_list_id ?><BR><BR>
	<table><tr>
	<td align=right><font class="body_text"> Kod wybierania: </td>
	<td align=left><font class="body_text"><input type=text size=7 maxlength=10 name=MDDiaLCodE class="cust_form" value="1">&nbsp; (dla polski będzie to 48)</td>
	</tr><tr>
	...
	<td align=right><font class="body_text"> Przeszukuj rekordy: </td>
	...
	<td align=right><font class="body_text"> Wybierz ten numer: </td>
	<td align=left><font class="body_text"><input type=text size=24 maxlength=20 name=MDDiaLOverridE class="cust_form" value="">&nbsp; (tylko cyfry)
	</td>
	</tr></table>
	<BR>
	...
	<a href="#" onclick="NeWManuaLDiaLCalLSubmiT('PREVIEW');return false;">Podgląd połączeń</a>
	 &nbsp;  &nbsp;  &nbsp;  &nbsp;  &nbsp;  &nbsp;  &nbsp;  &nbsp; 
	<a href="#" onclick="hideDiv('NeWManuaLDiaLBox');return false;">Wróć</a>
	</TD></TR></TABLE>
</span>
...
<span style="position:absolute;left:157px;top:<?php echo $HTheight ?>px;z-index:32;" id="TransferMain">
	...
	<span id="XfeRGrouPLisT"><select size=1 name=XfeRGrouP class="cust_form"><option>-- SELECT A GROUP TO SEND YOUR Rozmowa TO --</option></select></span>
	...
	<TD ALIGN=LEFT>
	<input type=checkbox name=consultativexfer size=1 value="0"><font class="body_tiny"> KONSULTACYJNA &nbsp;</font>
	</TD>
	<TD ALIGN=LEFT>
	...
	<TD ALIGN=LEFT>
	<input type=checkbox name=xferoverride size=1 value="0"><font class="body_tiny"> WYBIERZ TEN NUMER</font>
	</TD>
	<TD ALIGN=LEFT>
...
<span style="position:absolute;left:0px;top:<?php echo $CQheight ?>px;width:<?php echo $MNwidth ?>;overflow:scroll;z-index:23;background-color:<?php echo $SIDEBAR_COLOR ?>;" id="callsinqueuedisplay"><TABLE CELLPADDING=0 CELLSPACING=0 Border=0><TR><TD width=5 ROWSPAN=2>&nbsp;</TD><TD ALIGN=CENTER><font class="body_text">Wzywa w kolejce: &nbsp; </font></TD></TR><TR><TD ALIGN=CENTER><span id="callsinqueuelist">&nbsp;</span></TD></TR></TABLE></span>
...
<span style="position:absolute;left:0px;top:0px;z-index:48;" id="DispoButtonHideC">
    <table border=0 bgcolor="#CCFFCC" width=<?php echo $CAwidth ?> height=47><TR><TD align=center VALIGN=top>Zmiany wprowadzone w informacjach o kliencie będą zapisane tylko podczas trwania połączenia. Nie po rozłączaniu!. </TD></TR></TABLE>
</span>
...
<span style="position:absolute;left:0px;top:0px;z-index:49;" id="DispoSelectBox">
	...
	<BR><BR>
	<a href="#" onclick="WeBForMDispoSelect_submit();return false;">WEB FORM ZATWIERDŹ</a>
	<BR><BR> &nbsp; 
	</TD></TR></TABLE>
</span>
...
<span style="position:absolute;left:0px;top:0px;z-index:55;" id="NothingBox">
	...
	if ( ($outbound_autodial_active < 1) or ($disable_blended_checkbox > 0) or ($dial_method == 'INBOUND_MAN') )
		{echo "<input type=checkbox name=CloserSelectBlended size=1 value=\"0\"> KOMBINOWANE POŁĄCZENIA<BR>";}
	?>
</span>
...
<span style="position:absolute;left:<?php echo $HKwidth ?>px;top:<?php echo $HKheight ?>px;z-index:16;" id="hotkeysdisplay"><a href="#" onMouseOver="HotKeys('ON')"><IMG SRC="../agc/images/vdc_XB_hotkeysactive_OFF_pl.gif" border=0 alt="KLAWISZE SKRÓTU NIEAKTYWNE"></a></span>
<?php } ?>


<span style="position:absolute;left:0px;top:<?php echo $HKheight ?>px;z-index:15;" id="MaiNfooterspan">
<table BGCOLOR="<?php echo $MAIN_COLOR ?>" id="MaiNfooter" width=<?php echo $MNwidth ?>><tr height=32><td height=32><font face="Arial,Helvetica" size=1>Agent web-client version: <?php echo $version ?> &nbsp; &nbsp; KOMPILACJA: <?php echo $build ?> &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; Serwer: <?php echo $server_ip ?>  &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp;</font><BR>
...
<span id="DiaLDiaLAltPhonE"><font class="preview_text"> <input type=checkbox name=DiaLAltPhonE size=1 value="0"> WYBIERANIE ALT NUM<BR></font></span>
...
if ( ($alt_phone_dialing) and ($auto_dial_level==0) )
	{echo "<font class=\"preview_text\"> <input type=checkbox name=DiaLAltPhonE size=1 value=\"0\"> WYBIERANIE ALT NUM<BR></font>";}
...
<table><tr>
<td align=right></td>
<td align=left><font class="body_text">&nbsp; Customer Time: <input type=text size=27 name=custdatetime class="cust_form" value=""> &nbsp; Kanał: <input type=text size=27 name=callchannel class="cust_form" value=""></td>
</tr><tr>
...
<td align=right><font class="body_text">Płeć:</td>
<td align=left><font class="body_text"><span id="GENDERhideFORie"><select size=1 name=gender_list class="cust_form" id=gender_list><option value="U">U - Niezdefiniowana</option><option value="M">M - Male</option><option value="F">F - Female</option></select></span></td>
</tr><tr>
...
<td align=right valign=top><font class="body_text"> Komentarz: </td>
