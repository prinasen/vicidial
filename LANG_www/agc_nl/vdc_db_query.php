...
if ($ACTION == 'regCLOSER')
	...
	if ( (strlen($closer_choice)<1) || (strlen($user)<1) )
		{
		$channel_live=0;
		echo "Groep Keuze $closer_choice is niet geldig\n";
		exit;
		}
	else
		{
	...
	echo "Closer In Groep Keuze $closer_choice is geregistreerd voor gebruiker $user\n";
	}
		...
		if (strlen($phone_number)>3)
			...
			#	$DB=1;
				if (strlen($dial_statuses)>2)
					...
					if (eregi("UP LAST BEL TIME",$lead_order)){$order_stmt = 'order by last_local_call_time desc, lead_id asc';}
					if (eregi("DOWN LAST BEL TIME",$lead_order)){$order_stmt = 'order by last_local_call_time, lead_id asc';}
...
if ($ACTION == 'alt_phone_change')
{
	...
	if ( (strlen($stage)<1) || (strlen($called_count)<1) || (strlen($lead_id)<1)  || (strlen($phone_number)<1) )
		{
		$channel_live=0;
		echo "ALT TEL. NUMMER STATUS NOT CHANGED\n";
		...
		echo "ALT TEL. NUMMER STATUS CHANGED\n";
		}
}
...
if ($ACTION == 'VDADcheckINCOMING')
	...
	if ( (strlen($campaign)<1) || (strlen($server_ip)<1) )
		...
		if ($queue_leadID_ct > 0)
			...
			### If BELBK, change vicidial_callback record to INACTIVE
...
if ($ACTION == 'CALLSINQUEUEview')
	...
	if (eregi('NONE',$view_calls_in_queue))
		{
		echo "Roept in wachtrij Bekijk een handicap voor deze campagne\n";
		exit;
		}
	else
		{
		...
		echo "<TD BGCOLOR=\"#CCCCCC\"><font style=\"font-size:11px;font-family:sans-serif;\"><B> &nbsp; BEL GROUP &nbsp; </font></TD>";
...
if ($ACTION == 'CALLSINQUEUEgrab')
	...
	if ( (eregi('NONE',$view_calls_in_queue)) or (eregi('N',$grab_calls_in_queue)) )
		{
		echo "ERROR: Roept in wachtrij Bekijk een handicap voor deze campagne\n";
		exit;
		}
	else
		{
