...
if (strlen($group[0]) < 1)
	...
	$OUToutput .= "Llamadas dentro de esta campaña $TOTALcalls\n";
	$OUToutput .= "Longitud media para las llamadas en segundos: $average_call_seconds\n";
	if (eregi("YES",$include_rollover))
		{$OUToutput .= "Calls that went to rollover In-Grupo:         $inTOTALcalls\n";}


	$OUToutput .= "\n";
	$OUToutput .= "---------- RESPUESTA HUMANOSAS\n";
	...
	if (eregi("YES",$include_rollover))
		...
		if ( ($DROPcalls < 1) or ($CIcallsRAW < 1) )
			{$inDROPRESPUESTApercent = '0';}
		else
			{
			$inDROPRESPUESTApercent = (($DROPcallsRAW / $CIcallsRAW) * 100);
	...
	$OUToutput .= "Longitud media para las llamadas DROP en segundos:    $average_hold_seconds\n";
	...
	$OUToutput .= "\n";
	$OUToutput .= "---------- AUTO-DIAL SIN RESPUESTA\n";
	...
	$OUToutput .= "Longitud media para las Llamadas sin Respuesta en segundos:       $average_na_seconds\n";


	##############################
	#########  CONVOCATORIA colgar RAZÓN ESTADÍSTICAS

	$TOTALcalls = 0;

	$OUToutput .= "\n";
	$OUToutput .= "---------- CONVOCATORIA colgar RAZÓN ESTADÍSTICAS\n";
	$OUToutput .= "+----------------------+------------+\n";
	...
	$OUToutput .= "| STATUS | DESCRIPTION          | CATEGORÍA             | CALLS      | TOTAL TIME | AVG TIME |CALLS/HOUR|CALLS/HOUR|\n";
	$OUToutput .= "+--------+----------------------+----------------------+------------+------------+----------+----------+----------+\n";
...
#	$OUToutput .= "|   AGENT TIME                                                      | $aTOTALhours |                     | $aTOTALrate |\n";
	...
	##############################
	#########  ID DE LA LISTA BREAKDOWN STATS

	$TOTALcalls = 0;

	$OUToutput .= "\n";
	$OUToutput .= "---------- ID DE LA LISTA STATS\n";
	$OUToutput .= "+------------------------------------------+------------+\n";
	...
	$OUToutput .= "\n";
	$OUToutput .= "---------- CUSTOM STATUS CATEGORÍA STATS\n";
	$OUToutput .= "+----------------------+------------+--------------------------------+\n";
	...
	if ($costformat > 0)
		...
		echo "SALIENTE $query_date to $end_date, &nbsp; $rawTOTtalk_min minutes at \$$outbound_rate = \$$outbound_cost</B>\n";
