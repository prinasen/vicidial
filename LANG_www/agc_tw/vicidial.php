...
$agentcallsstatus		= '0';	# set to 1 to show agent status and call dialed count
   $campagentstatctmax	= '3';	# Number of秒 for campaign call and agent stats
...
$電話SComPIP			= '1';	# set to 1 to log computer IP to phone if blank, set to 2 to force log each login
			...
			if (xmlhttp) 
				...
				xmlhttp.onreadystatechange = function() 
					...
					//	alert(xmlhttp.responseText);
						...
						document.getElementById("LogiNReseT").innerHTML = "<INPUT TYPE=BUTTON VALUE=\"更新 活動 List\" OnClick=\"login_allowable_campaigns()\">";
...
echo "<TD ALIGN=LEFT VALIGN=BOTTOM><IMG SRC=\"../agc/images/vdc_tab_vicidial.gif\" 邊界=0></TD>";
...
echo "<span id=\"LogiNReseT\"><INPUT TYPE=BUTTON VALUE=\"更新 活動 List\" OnClick=\"login_allowable_campaigns()\"></span></TD></TR>\n";
...
echo "<TD WIDTH=100 ALIGN=RIGHT VALIGN=TOP  NOWRAP><a href=\"../agc_en/vicidial.php?relogin=YES&VD_login=$VD_login&VD_campaign=$VD_campaign&phone_login=$phone_login&phone_pass=$phone_pass&VD_pass=$VD_pass\">English <img src=\"../agc/images/en.gif\" BORDER=0 HEIGHT=14 WIDTH=20></a></TD>\n";echo "<TD WIDTH=100 ALIGN=RIGHT VALIGN=TOP  BGCOLOR=\"#CCFFCC\" NOWRAP><a href=\"../agc_tw/vicidial.php?relogin=YES&VD_login=$VD_login&VD_campaign=$VD_campaign&phone_login=$phone_login&phone_pass=$phone_pass&VD_pass=$VD_pass\">中文繁體 <img src=\"../agc/images/tw.gif\" BORDER=0 HEIGHT=14 WIDTH=20></a></TD>\n";	echo "</TR></TABLE>\n";
	...
	else
	{
		...
		echo "<title>Agent web client: 登入</title>\n";
		echo "</head>\n";
		echo "<BODY BGCOLOR=WHITE MARGINHEIGHT=0 MARGINWIDTH=0 onResize=\"browser_dimensions();\"  onLoad=\"browser_dimensions();\">\n";
		echo "<A HREF=\"./timeclock.php?referrer=agent&pl=$phone_login&pp=$phone_pass&VD_login=$VD_login&VD_pass=$VD_pass\"> Timeclock</A><BR>\n";
		echo "<TABLE WIDTH=100%><TR><TD></TD>\n";
...
$VDloginDISPLAY=0;
	...
	if($auth>0)
		...
		if ( (ereg('Y',$forced_timeclock_login)) or ( (ereg('ADMIN_EXEMPT',$forced_timeclock_login)) and ($VU_user_level < 8) ) )
			...
			if ( (strlen($last_agent_event)<2) or (ereg('登出',$last_agent_event)) )
				{
				$VDloginDISPLAY=1;
		...
		if ( ( (ereg("開始|ALL",$shift_enforcement)) and (!ereg("OFF",$VU_agent_shift_enforcement_override)) ) or (ereg("開始|ALL",$VU_agent_shift_enforcement_override)) )
			{
			$shift_ok=0;
			...
			if ( ($shift_ok < 1) and ($VU_shift_override_flag < 1) and ($VDloginDISPLAY > 0) )
				...
				$VDdisplayMESSAGE.= "<INPUT TYPE=Submit NAME=確認送出 VALUE=確認送出></FORM>\n";
				}
			}
...
echo "<TD WIDTH=100 ALIGN=RIGHT VALIGN=TOP  NOWRAP><a href=\"../agc_en/vicidial.php?relogin=YES&VD_login=$VD_login&VD_campaign=$VD_campaign&phone_login=$phone_login&phone_pass=$phone_pass&VD_pass=$VD_pass\">English <img src=\"../agc/images/en.gif\" BORDER=0 HEIGHT=14 WIDTH=20></a></TD>\n";echo "<TD WIDTH=100 ALIGN=RIGHT VALIGN=TOP  BGCOLOR=\"#CCFFCC\" NOWRAP><a href=\"../agc_tw/vicidial.php?relogin=YES&VD_login=$VD_login&VD_campaign=$VD_campaign&phone_login=$phone_login&phone_pass=$phone_pass&VD_pass=$VD_pass\">中文繁體 <img src=\"../agc/images/tw.gif\" BORDER=0 HEIGHT=14 WIDTH=20></a></TD>\n";			echo "</TR></TABLE>\n";
		...
		if($CAMPactive>0)
			...
			if (preg_match("/IN_GROUP|P重置_1|P重置_2|P重置_3|P重置_4|P重置_5/",$quick_transfer_button))
				{$quick_transfer_button_enabled=1;}

			$preset_populate='';
			$prepopulate_transfer_preset_enabled=0;
			if (preg_match("/P重置_1|P重置_2|P重置_3|P重置_4|P重置_5/",$prepopulate_transfer_preset))
				{
				$prepopulate_transfer_preset_enabled=1;
				if (preg_match("/P重置_1/",$prepopulate_transfer_preset))
					{$preset_populate = $xferconf_a_number;}
				if (preg_match("/P重置_2/",$prepopulate_transfer_preset))
					{$preset_populate = $xferconf_b_number;}
				if (preg_match("/P重置_3/",$prepopulate_transfer_preset))
					{$preset_populate = $xferconf_c_number;}
				if (preg_match("/P重置_4/",$prepopulate_transfer_preset))
					{$preset_populate = $xferconf_d_number;}
				if (preg_match("/P重置_5/",$prepopulate_transfer_preset))
			...
			if ( (!ereg('DISABLED',$VU_vicidial_recording_override)) and ($VU_vicidial_recording > 0) )
				...
				echo "<!-- USER 錄音ING OVERRIDE: |$VU_vicidial_recording_override|$campaign_recording| -->\n";
				}
...
### logged in and use that phone login account
	...
	if ( (strlen($park_ext)>0) && (strlen($park_file_name)>0) )
		...
		echo "<!-- 專案活動 CUSTOM 駐留ING:  |$VICIDiaL_park_on_extension|$VICIDiaL_park_on_filename| -->\n";
		}
	echo "<!-- 專案活動 DEFAULT 駐留ING: |$VICIDiaL_park_on_extension|$VICIDiaL_park_on_filename| -->\n";
	...
	if ( ( ($campaign_allow_inbound == 'Y') and ($dial_method != 'MANUAL') ) || ($campaign_leads_to_call > 0) || (ereg('Y',$no_hopper_leads_logins)) )
		...
		echo "<!-- 舊的佇列和INCALL回覆列表:   |$affected_rows| -->\n";
		...
		$affected_rows = mysql_affected_rows($link);
		echo "<!-- 舊的佇列和INCALL回覆 Hopper: |$affected_rows| -->\n";
		...
		$affected_rows = mysql_affected_rows($link);
		echo "<!-- 值機人員錄音檔清理完畢: |$affected_rows| -->\n";
...
$SCwidth =  ($MASTERwidth + 230);	# 670 - live call秒 counter, sidebar link
...
$Cmonths = Array('一月','February','三月','四月','五月','六月',
				'七月','八月','九月','十月','十一月','十二月');
	...
	function volume_control(taskdirection,taskvolchannel,taskagentmute) 
		...
		if (taskagentmute=='AgenT')
			...
			if (taskdirection=='MUTING')
				{
				document.getElementById("AgentMuteSpan").innerHTML = "<a href=\"#CHAN-" + agentchannel + "\" onclick=\"volume_control('UNMUTE','" + agentchannel + "','AgenT');return false;\"><IMG SRC=\"../agc/images/vdc_volume_UNMUTE.gif\" 邊界=0></a>";
				}
			else
				{
				document.getElementById("AgentMuteSpan").innerHTML = "<a href=\"#CHAN-" + agentchannel + "\" onclick=\"volume_control('MUTING','" + agentchannel + "','AgenT');return false;\"><IMG SRC=\"../agc/images/vdc_volume_MUTE.gif\" 邊界=0></a>";
				}
			}
				...
				xmlhttprequestcheckconf.onreadystatechange = function() 
					...
					//	alert(xmlhttprequestcheckconf.responseText);
						...
						if ( (callholdstatus == '1') || (agentcallsstatus == '1') || (vicidial_agent_disable != 'NOT_ACTIVE') )
							...
							if (AGLogiN != 'N')
								{
								document.getElementById("Agent狀態狀態").innerHTML = AGLogiN;
								}
							if (CamPCalLs != 'N')
								{
								document.getElementById("Agent狀態Calls").innerHTML = CamPCalLs;
								}
							if (DiaLCalLs != 'N')
								{
								document.getElementById("Agent狀態DiaLs").innerHTML = DiaLCalLs;
								}
						...
						var VLAStatuS_array = check_time_array[4].split("狀態: ");
						...
						if (InGroupChange > 0)
							...
							var external_igb_set_user = InGroupChange使用者;
...
// ZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZ  above section working on API functions
						...
						if ( (conf_channels_xtra_display == 1) || (conf_channels_xtra_display == 0) )
							...
							if (live_conf_calls > 0)
								...
								agentphonelive=0;
								var conv_start=-1;
								var live_conf_HTML = "<font face=\"Arial,Helvetica\"><B>您通話中的號碼:</B></font><BR><TABLE WIDTH=<?php echo $CQwidth ?>><TR BGCOLOR=<?php echo $SCRIPT_COLOR ?>><TD><font class=\"log_title\">#</TD><TD><font class=\"log_title\">遠端通道</TD><TD><font class=\"log_title\">掛斷</TD><TD><font class=\"log_title\">音量</TD></TR>";
									...
									if ( (HidEMonitoRSessionS==1) && (channelfieldA.match(/ASTblind/)) )
										...
										if (channelfieldA.match(regRNnolink))
											...
											if (volumecontrol_active!=1)
												{
												live_conf_HTML = live_conf_HTML + "<tr bgcolor=\"" + row_color + "\"><td><font class=\"log_text\">" + loop_ct + "</td><td><font class=\"" + chan_name_color + "\">" + channelfieldA + "</td><td><font class=\"log_text\"><a href=\"#\" onclick=\"livehangup_send_hangup('" + channelfieldA + "');return false;\">掛斷</a></td><td></td></tr>";
												}
											else
												{
												live_conf_HTML = live_conf_HTML + "<tr bgcolor=\"" + row_color + "\"><td><font class=\"log_text\">" + loop_ct + "</td><td><font class=\"" + chan_name_color + "\">" + channelfieldA + "</td><td><font class=\"log_text\"><a href=\"#\" onclick=\"livehangup_send_hangup('" + channelfieldA + "');return false;\">掛斷</a></td><td><a href=\"#\" onclick=\"volume_control('UP','" + channelfieldA + "','');return false;\"><IMG SRC=\"../agc/images/vdc_volume_up.gif\" 邊界=0></a> &nbsp; <a href=\"#\" onclick=\"volume_control('DOWN','" + channelfieldA + "','');return false;\"><IMG SRC=\"../agc/images/vdc_volume_down.gif\" 邊界=0></a> &nbsp; &nbsp; &nbsp; <a href=\"#\" onclick=\"volume_control('MUTING','" + channelfieldA + "','');return false;\"><IMG SRC=\"../agc/images/vdc_volume_MUTE.gif\" 邊界=0></a> &nbsp; <a href=\"#\" onclick=\"volume_control('UNMUTE','" + channelfieldA + "','');return false;\"><IMG SRC=\"../agc/images/vdc_volume_UNMUTE.gif\" 邊界=0></a></td></tr>";
												}
											}
										}
...
// Covers the following types: XFER, VMAIL, ENTRY, CONF, 駐留, FROM駐留, XfeRLOCAL, XfeRINTERNAL, XfeRBLIND, VfeRVMAIL
		...
		if (xmlhttpXF) 
			...
			if ( (taskvar == 'XfeRBLIND') || (taskvar == 'XfeRVMAIL') )
				...
				if (blindxferdialstring.length<'2')
					...
					xferredirect_query='';
					taskvar = 'NOTHING';
					alert("轉接號碼必須超過一個數字:" + blindxferdialstring);
					}
				else
					{
		...
		if (taskMDstage == "start") 
			...
			if (alt_phone_dialing == 1)
				...
				if (document.vicidial_form.DiaLAltPhonE.checked==true)
					...
					var man_status = "撥打其他電話號碼: <a href=\"#\" onclick=\"ManualDialOnly('MaiNPhonE')\"><font class=\"preview_text\">主要電話</font></a> or <a href=\"#\" onclick=\"ManualDialOnly('ALT電話E')\"><font class=\"preview_text\">其他電話</font></a> or <a href=\"#\" onclick=\"ManualDialOnly('AddresS3')\"><font class=\"preview_text\">地址3</font></a> or <a href=\"#\" onclick=\"ManualDialAltDonE()\"><font class=\"preview_text_red\">結束紀錄</font></a>"; 
			...
			xmlhttp.onreadystatechange = function() 
				...
				//	alert(xmlhttp.responseText);
					...
					if (CBcounT == 0) {var CBprint = "NO";}
						...
						document.getElementById("CBstatusSpan").innerHTML ="<a href=\"#\" onclick=\"CalLBacKsLisTCheck();return false;\">" + CBprint + " 生效的回撥電話</a>";
			...
			if (xmlhttp) 
				...
				xmlhttp.onreadystatechange = function() 
					...
					//	alert(xmlhttp.responseText);
						...
						var loop_ct=0;
						var conv_start=0;
						var CB_HTML = "<table width=610><tr bgcolor=<?php echo $SCRIPT_COLOR ?>><td><font class=\"log_title\">#</td><td><font class=\"log_title\"> 撥號BACK DATE/TIME</td><td><font class=\"log_title\">號碼</td><td><font class=\"log_title\">NAME</td><td><font class=\"log_title\">  狀態</td><td align=right><font class=\"log_title\">CAMPAIGN</td><td><font class=\"log_title\">LAST 通話日期\/時間</td><td align=left><font class=\"log_title\"> COMMENTS</td></tr>"
...
// Fast version of manual dial
		...
		if ( (MDDiaLCodEform.length < 1) || (MDPhonENumbeRform.length < 5) )
			{
			alert("您必須輸入電話號碼和 WÄHLEN CODE 以使用快速撥號");
			}
		else
			{
			var MDLookuPLeaD = 'new';
			...
			xmlhttp.onreadystatechange = function() 
				...
				//	alert(xmlhttp.responseText);
					...
					//		alert(document.getElementById("MainStatuSSpan").innerHTML);
							document.getElementById("MainStatuSSpan").innerHTML = " 撥號中: " + status_display_number + " UID: " + CIDcheck + " &nbsp; 進入閒置狀態... " + MD_ring_secondS + "秒";
					//		alert("channel not found yet:\n" + campaign);
						...
						if (taskCheckOR == 'YES')
							...
							if ( (XDchannel.match(regMDL)) && (asterisk_version != '1.0.8') && (asterisk_version != '1.0.9') && (MD_ring_secondS < 10) )
								...
								document.getElementById("MainStatuSSpan").innerHTML = " 撥給第三方: " + document.vicidial_form.xfernumber.value + " UID: " + CIDcheck;
							...
							if ( (MDchannel.match(regMDL)) && (asterisk_version != '1.0.8') && (asterisk_version != '1.0.9') )
								...
								document.getElementById("MainStatuSSpan").innerHTML = " 已撥號: " + status_display_number + " UID: " + CIDcheck + " &nbsp;"; 
								...
								document.getElementById("VolumeUpSpan").innerHTML = "<a href=\"#\" onclick=\"volume_control('UP','" + MDchannel + "','');return false;\"><IMG SRC=\"../agc/images/vdc_volume_up.gif\" 邊界=0></a>";
								document.getElementById("VolumeDownSpan").innerHTML = "<a href=\"#\" onclick=\"volume_control('DOWN','" + MDchannel + "','');return false;\"><IMG SRC=\"../agc/images/vdc_volume_down.gif\" 邊界=0></a>";

								if (quick_transfer_button == 'IN_GROUP')
									{
									document.getElementById("QuickXfer").innerHTML = "<a href=\"#\" onclick=\"mainxfer_send_redirect('XfeRLOCAL','" + lastcustchannel + "','" + lastcustserverip + "');return false;\"><IMG SRC=\"../agc/images/vdc_LB_quickxfer.gif\" border=0 alt=\"快速傳遞\"></a>";
									}
								if (prepopulate_transfer_preset_enabled > 0)
									{
									if (prepopulate_transfer_preset == 'P重置_1')
										{document.vicidial_form.xfernumber.value = CalL_XC_a_NuMber;}
									if (prepopulate_transfer_preset == 'P重置_2')
										{document.vicidial_form.xfernumber.value = CalL_XC_b_NuMber;}
									if (prepopulate_transfer_preset == 'P重置_3')
										{document.vicidial_form.xfernumber.value = CalL_XC_c_NuMber;}
									if (prepopulate_transfer_preset == 'P重置_4')
										{document.vicidial_form.xfernumber.value = CalL_XC_d_NuMber;}
									if (prepopulate_transfer_preset == 'P重置_5')
										{document.vicidial_form.xfernumber.value = CalL_XC_e_NuMber;}
									}
								if ( (quick_transfer_button == 'P重置_1') || (quick_transfer_button == 'P重置_2') || (quick_transfer_button == 'P重置_3') || (quick_transfer_button == 'P重置_4') || (quick_transfer_button == 'P重置_5') )
									{
									document.getElementById("QuickXfer").innerHTML = "<a href=\"#\" onclick=\"mainxfer_send_redirect('XfeRBLIND','" + lastcustchannel + "','" + lastcustserverip + "');return false;\"><IMG SRC=\"../agc/images/vdc_LB_quickxfer.gif\" border=0 alt=\"快速傳遞\"></a>";

									if (quick_transfer_button == 'P重置_1')
										{document.vicidial_form.xfernumber.value = CalL_XC_a_NuMber;}
									if (quick_transfer_button == 'P重置_2')
										{document.vicidial_form.xfernumber.value = CalL_XC_b_NuMber;}
									if (quick_transfer_button == 'P重置_3')
										{document.vicidial_form.xfernumber.value = CalL_XC_c_NuMber;}
									if (quick_transfer_button == 'P重置_4')
										{document.vicidial_form.xfernumber.value = CalL_XC_d_NuMber;}
									if (quick_transfer_button == 'P重置_5')
										{document.vicidial_form.xfernumber.value = CalL_XC_e_NuMber;}
									}
...
// Send the Manual 撥下一個號碼 request
	function ManualDialNext(mdnCBid,mdnBDleadid,mdnDiaLCodE,mdnPhonENumbeR,mdnStagE,mdVendorid,mdgroupalias)
		...
		if (document.vicidial_form.LeadPreview.checked==true)
			...
			var man_preview = 'YES';
			var man_status = "預覽資料後 <a href=\"#\" onclick=\"ManualDialOnly()\"><font class=\"preview_text\"> WÄHLEN LEAD</font></a> or <a href=\"#\" onclick=\"ManualDialSkip()\"><font class=\"preview_text\">SKIP LEAD</font></a>"; 
			}
		else
			{
		...
		if (xmlhttp) 
			...
			//		alert(xmlhttp.responseText);
					...
					//	CalL_ScripT_id									= MDnextResponse_array[37];
						...
						document.getElementById("MainStatuSSpan").innerHTML = " 撥號中: " + status_display_number + " UID: " + MDnextCID + " &nbsp; " + man_status;
						...
						if (LeaDPreVDispO == 'CALLBK')
							{
							document.getElementById("CusTInfOSpaN").innerHTML = " <B> PREVIOUS 撥號BACK </B>";
...
// Send the Manual Dial Only - dial the previewed lead
	function ManualDialOnly(taskaltnum)
		...
		if (dialed_label == 'ALT')
			{document.getElementById("CusTInfOSpaN").innerHTML = " <B> 其他撥打號碼: ALT </B>";}
		...
		if (xmlhttp) 
			...
			//		alert(xmlhttp.responseText);
					...
					if (MDnextCID == " 撥號 NOT PLACED")
						...
						if (alt_dial_status_display=='0')
							{
							document.getElementById("MainStatuSSpan").innerHTML = " 撥號中: " + status_display_number + " UID: " + MDnextCID + " &nbsp; 進入閒置狀態...";
...
// Set the client to READY and start looking for calls (VDADready, VDADpause)
	function AutoDial_ReSume_PauSe(taskaction,taskagentlog,taskwrapup,taskstatuschange,temp_reason)
		...
		if (taskaction == 'VDADready')
			...
			if ( (agent_pause_codes_active=='FORCE') && (temp_reason != '登出') && (temp_reason != 'REQUEUE') && (temp_reason != 'DIALNEXT') )
				{
					...
					//	alert(xmlhttprequestcheckauto.responseText);
						...
						//	DialLog("start");
							...
							if (lastcustserverip == server_ip)
								{
								document.getElementById("VolumeUpSpan").innerHTML = "<a href=\"#\" onclick=\"volume_control('UP','" + lastcustchannel + "','');return false;\"><IMG SRC=\"../agc/images/vdc_volume_up.gif\" 邊界=0></a>";
								document.getElementById("VolumeDownSpan").innerHTML = "<a href=\"#\" onclick=\"volume_control('DOWN','" + lastcustchannel + "','');return false;\"><IMG SRC=\"../agc/images/vdc_volume_down.gif\" 邊界=0></a>";
								}
...
// Send Hangup command for customer call connected to the conference now to Manager
	function dialedcall_send_hangup(dispowindow,hotkeysused,altdispo,nodeletevdac) 
		...
		//  DEACTIVATE CHANNEL-DEPENDANT BUTTONS AND VARIABLES
			...
			document.getElementById("VolumeUpSpan").innerHTML = "<IMG SRC=\"../agc/images/vdc_volume_up_off.gif\" 邊界=0>";
			document.getElementById("VolumeDownSpan").innerHTML = "<IMG SRC=\"../agc/images/vdc_volume_down_off.gif\" 邊界=0>";
...
// Generate the Call Disposition Chooser panel
	function DispoSelectContent_create(taskDSgrp,taskDSstage)
		...
		var dispo_HTML = "<table cellpadding=5 cellspacing=5 width=500><tr><td colspan=2><B> 撥號 DISPOSITION</B></td></tr><tr><td bgcolor=\"#99FF99\" height=300 width=240 valign=top><font class=\"log_text\"><span id=DispoSelectA>";
		var loop_ct = 0;
...
// open web form, then submit disposition
	function WeBForMDispoSelect_submit()
		...
		if (DispoChoice.length < 1) {alert("請選擇結束碼");}
		else
			{
...
// Generate the Closer In Group Chooser panel
	function CloserSelectContent_create()
		...
		if ( (VU_agent_choose_ingroups == '1') && (manager_ingroups_set < 1) )
			{
			var live_CSC_HTML = "<table cellpadding=5 cellspacing=5 width=500><tr><td><B>未選擇群組</B></td><td><B>已選群組</B></td></tr><tr><td bgcolor=\"#99FF99\" height=300 width=240 valign=top><font class=\"log_text\"><span id=CloserSelectAdd> &nbsp; <a href=\"#\" onclick=\"CloserSelect_change('-----ADD-ALL-----','ADD');return false;\"><B>--- ADD ALL ---</B><BR>";
			var loop_ct = 0;
	...
	function BrowserCloseLogout()
		...
		if (logout_stop_timeouts < 1)
			...
			if (VDRP_stage != 'PAUSED')
				{
				AutoDial_ReSume_PauSe("VDADpause",'','','',"登出");
				}
			LogouT('CLOSE');
			alert("下次登出時請點擊登出連結.\n");
			}
...
// ################################################################################
// 更新 the agents view sidebar or xfer frame
...
// ################################################################################
// 更新 the calls in queue bottombar
	...
	function OpeNGrouPSelectioN()
		...
		if ( (AutoDialWaiting == 1) || (VD_live_customer_call==1) || (alt_dial_active==1) )
			...
			if (manager_ingroups_set > 0)
				{
				alert("經理" + external_igb_set_name + " 您的選擇組的選擇");
				}
			else
				{
				HidEGenDerPulldown();
	...
	function start_all_refresh()
		...
		//	if ( (agentcall_manual != '1') && (starting_dial_level > 0) )
			...
			if ( (VtigeRLogiNScripT == 'Y') && (VtigeREnableD > 0) )
				{
				document.getElementById("ScriptContents").innerHTML = "<iframe src=\"" + VtigeRurl + "/index.php?module=使用者s&action=Authenticate&return_module=使用者s&return_action=登入&user_name=" + user + "&user_password=" + pass + "&login_theme=softed&login_language=en_us\" style=\"background-color:transparent;\" scrolling=\"auto\" frameborder=\"0\" allowtransparency=\"true\" id=\"popupFrame\" name=\"popupFrame\" width=\"" + script_width + "\" height=\"" + script_height + "\" STYLE=\"z-index:17\"> </iframe> ";
				}
			if ( (VtigeRLogiNScripT == 'NEW_WINDOW') && (VtigeREnableD > 0) )
				{
				var VtigeRall = VtigeRurl + "/index.php?module=使用者s&action=Authenticate&return_module=使用者s&return_action=登入&user_name=" + user + "&user_password=" + pass + "&login_theme=softed&login_language=en_us";
			...
			if (WaitingForNextStep==0)
				...
				if (agent_status_view_active > 0)
					{
					refresh_agents_view('AgentView狀態',agent_status_view);
					}
		...
		if (VU_vicidial_transfers == '1')
			...
			if (showxfervar == 'ON')
				...
				if (showoffvar == 'YES')
					...
					if ( (quick_transfer_button == 'P重置_1') || (quick_transfer_button == 'P重置_2') )
						{
						document.getElementById("QuickXfer").innerHTML = "<a href=\"#\" onclick=\"mainxfer_send_redirect('XfeRBLIND','" + lastcustchannel + "','" + lastcustserverip + "');return false;\"><IMG SRC=\"../agc/images/vdc_LB_quickxfer.gif\" border=0 alt=\"快速傳遞\"></a>";
						}
					}
...
<TABLE 邊界=0 CELLPADDING=0 CELLSPACING=0 BGCOLOR=white WIDTH=<?php echo $MNwidth ?> MARGINWIDTH=0 MARGINHEIGHT=0 LEFTMARGIN=0 TOPMARGIN=0 VALIGN=TOP ALIGN=LEFT>
<TR VALIGN=TOP ALIGN=LEFT><TD COLSPAN=3 VALIGN=TOP ALIGN=LEFT>
<INPUT TYPE=HIDDEN NAME=extension>
<font class="queue_text">
<?php	echo "以使用者身份登入: $VD_login on 電話: $SIP_user to campaign: $VD_campaign&nbsp; \n"; ?>
...
<TD ALIGN=LEFT WIDTH=115><A HREF="#" onclick="MainPanelToFront('NO');"><IMG SRC="../agc/images/vdc_tab_vicidial.gif" ALT="MAIN" WIDTH=115 HEIGHT=30 邊界=0></A></TD>
<TD ALIGN=LEFT WIDTH=105><A HREF="#" onclick="ScriptPanelToFront();"><IMG SRC="../agc/images/vdc_tab_script.gif" ALT="SCRIPT" WIDTH=105 HEIGHT=30 邊界=0></A></TD>
<TD WIDTH=<?php echo $HSwidth ?> VALIGN=MIDDLE ALIGN=CENTER><font class="body_text">&nbsp; <span id=status>LIVE</span>&nbsp; &nbsp;session ID: <span id=sessionIDspan></span>&nbsp; &nbsp;<span id=Agent狀態Calls></span></TD>
<TD WIDTH=109><IMG SRC="../agc/images/agc_live_call_OFF.gif" NAME=livecall ALT="通話中電話" WIDTH=109 HEIGHT=30 邊界=0></TD>
</TR></TABLE>
</span>



<span style="position:absolute;left:0px;top:0px;z-index:3;" id="歡迎BoxA">
    <table border=0 bgcolor="#FFFFFF" width=<?php echo $CAwidth ?> height=<?php echo $HKwidth ?>><TR><TD align=center><BR><span id="歡迎BoxAt">Agent Screen</span></TD></TR></TABLE>
</span>
...
<span style="position:absolute;left:<?php echo $SCwidth ?>px;top:49px;z-index:18;" id="SecondSspan"><font class="body_text">秒: 
...
<span style="position:absolute;left:0px;top:0px;z-index:51;" id="CallBacKsLisTBox">
	...
	<a href="#" onclick="CalLBacKsLisTCheck();return false;">更新</a>
	 &nbsp;  &nbsp;  &nbsp;  &nbsp;  &nbsp;  &nbsp;  &nbsp;  &nbsp; 
	<a href="#" onclick="hideDiv('CallBacKsLisTBox');return false;">回上一頁</a>
	</TD></TR></TABLE>
</span>
...
<span style="position:absolute;left:0px;top:0px;z-index:52;" id="NeWManuaLDiaLBox">
	...
	if (eregi("X",dial_prefix))
		{
		echo "Note: a dial prefix of $dial_prefix 將會加在此號碼之前<BR>\n";
		}
	?>
	...
	<td align=right><font class="body_text"> 電話 Number: </td>
	...
	<td align=right><font class="body_text"> 搜尋存在的紀錄: </td>
	<td align=left><font class="body_text"><input type=checkbox name=LeadLookuP size=1 value="0">&nbsp; (本選項勾選時，在增加新的紀錄時會尋找系統中的電話號碼)</td>
	</tr><tr>
	...
	<td align=left><font class="body_text"><input type=text size=24 maxlength=20 name=MDDiaLOverridE class="cust_form" value="">&nbsp; (限數字碼輸入)
	</td>
	</tr></table>
	<BR>
	...
	<a href="#" onclick="NeWManuaLDiaLCalLSubmiT('PREVIEW');return false;">預覽電話</a>
	 &nbsp;  &nbsp;  &nbsp;  &nbsp;  &nbsp;  &nbsp;  &nbsp;  &nbsp; 
	<a href="#" onclick="hideDiv('NeWManuaLDiaLBox');return false;">回上一頁</a>
	</TD></TR></TABLE>
</span>



<span style="position:absolute;left:5px;top:<?php echo $CBheight ?>px;z-index:19;" id="VolumeControlSpan"><span id="VolumeUpSpan"><IMG SRC="../agc/images/vdc_volume_up_off.gif" 邊界=0></span><BR><span id="VolumeDownSpan"><IMG SRC="../agc/images/vdc_volume_down_off.gif" 邊界=0></span>
</font></span>



<span style="position:absolute;left:35px;top:<?php echo $CBheight ?>px;z-index:20;" id="Agent狀態Span"><font class="body_text">
您目前的狀態: <span id="Agent狀態狀態"></span> <BR>Calls Dialing: <span id="Agent狀態DiaLs"></span> 
</font></span>
...
<span style="position:absolute;left:157px;top:<?php echo $HTheight ?>px;z-index:32;" id="TransferMain">
	...
	<span id="XfeRGrouPLisT"><select size=1 name=XfeRGrouP class="cust_form"><option>-- SELECT A GROUP TO SEND YOUR 撥號 TO --</option></select></span>
	...
	<TR>
	<TD ALIGN=LEFT COLSPAN=2>
	<IMG SRC="../agc/images/vdc_XB_seconds_tw.gif" border=0 alt="seconds" style="vertical-align:middle"><input type=text size=2 name=xferlength maxlength=4 class="cust_form">
	&nbsp; 
	<IMG SRC="../agc/images/vdc_XB_channel_tw.gif" border=0 alt="channel" style="vertical-align:middle"><input type=text size=12 name=xferchannel maxlength=200 class="cust_form">
	</TD>
	<TD ALIGN=LEFT>
	<input type=checkbox name=consultativexfer size=1 value="0"><font class="body_tiny"> 協商 &nbsp;</font>
	</TD>
	<TD ALIGN=LEFT>
	...
	<TD ALIGN=LEFT>
	<input type=checkbox name=xferoverride size=1 value="0"><font class="body_tiny"> 重撥</font>
	</TD>
	<TD ALIGN=LEFT>
...
<span style="position:absolute;left:0px;top:<?php echo $CQheight ?>px;width:<?php echo $MNwidth ?>;overflow:scroll;z-index:23;background-color:<?php echo $SIDEBAR_COLOR ?>;" id="callsinqueuedisplay"><TABLE CELLPADDING=0 CELLSPACING=0 邊界=0><TR><TD width=5 ROWSPAN=2>&nbsp;</TD><TD ALIGN=CENTER><font class="body_text">呼籲在隊列: &nbsp; </font></TD></TR><TR><TD ALIGN=CENTER><span id="callsinqueuelist">&nbsp;</span></TD></TR></TABLE></span>
...
<span style="position:absolute;left:<?php echo $SBwidth ?>px;top:0px;height:500;overflow:scroll;z-index:25;background-color:<?php echo $SIDEBAR_COLOR ?>;" id="AgentViewSpan"><TABLE CELLPADDING=0 CELLSPACING=0 邊界=0><TR><TD width=5 ROWSPAN=2>&nbsp;</TD><TD ALIGN=CENTER><font class="body_text">
其他代理狀態: &nbsp; </font></TD></TR><TR><TD ALIGN=CENTER><span id="AgentView狀態">&nbsp;</span></TD></TR></TABLE></span>
...
<span style="position:absolute;left:<?php echo $SCwidth ?>px;top:<?php echo $SLheight ?>px;z-index:27;" id="AgentViewLinkSpan"><TABLE CELLPADDING=0 CELLSPACING=0 邊界=0 WIDTH=91><TR><TD ALIGN=RIGHT><font class="body_small"><span id="AgentViewLink"><a href="#" onclick="AgentsViewOpen('AgentViewSpan','open');return false;">Agents View +</a></span></font></TD></TR></TABLE></span>
...
<span style="position:absolute;left:5px;top:<?php echo $HTheight ?>px;z-index:34;" id="HotKeyActionBox">
	...
	<TR bgcolor="#FFEEBB"><TD height=70><font class="sh_text"> 設定結束碼為: </font><BR><BR><CENTER>
...
<span style="position:absolute;left:5px;top:<?php echo $HTheight ?>px;z-index:35;" id="HotKeyEntriesBox">
    <table border=0 bgcolor="#FFDD99" width=<?php echo $HCwidth ?> height=70>
	<TR bgcolor="#FFEEBB"><TD width=200><font class="sh_text"> 結束碼快捷鍵: </font></td><td colspan=2>
...
<span style="position:absolute;left:0px;top:0px;z-index:48;" id="DispoButtonHideC">
    <table border=0 bgcolor="#CCFFCC" width=<?php echo $CAwidth ?> height=47><TR><TD align=center VALIGN=top>你必須先更改客戶資料，再掛斷電話。否則所做變更將不會生效！. </TD></TR></TABLE>
</span>
...
<span style="position:absolute;left:0px;top:0px;z-index:49;" id="DispoSelectBox">
	...
	<BR><BR>
	<a href="#" onclick="WeBForMDispoSelect_submit();return false;">WEB FORM 確認送出</a>
	<BR><BR> &nbsp; 
	</TD></TR></TABLE>
</span>
...
<span style="position:absolute;left:0px;top:0px;z-index:53;" id="CloserSelectBox">
	...
	if ( ($outbound_autodial_active > 0) and ($disable_blended_checkbox < 1) and ($dial_method != 'INBOUND_MAN') )
		{
		?>
		<input type=checkbox name=CloserSelectBlended size=1 value="0"> INBOUND/OUTBOUND BLENDING(啟動外撥) <BR>
		<?php
		}
	?>
...
<span style="position:absolute;left:0px;top:0px;z-index:55;" id="NothingBox">
	...
	<?php
	if (!$agentonly_callbacks)
		{echo "<input type=checkbox name=CallBackOnlyMe size=1 value=\"0\"> 只有個人回撥 <BR>";}
	if ( ($outbound_autodial_active < 1) or ($disable_blended_checkbox > 0) or ($dial_method == 'INBOUND_MAN') )
		{echo "<input type=checkbox name=CloserSelectBlended size=1 value=\"0\"> INBOUND/OUTBOUND BLENDING<BR>";}
	?>
</span>
...
<span style="position:absolute;left:<?php echo $HKwidth ?>px;top:<?php echo $HKheight ?>px;z-index:16;" id="hotkeysdisplay"><a href="#" onMouseOver="HotKeys('ON')"><IMG SRC="../agc/images/vdc_XB_hotkeysactive_OFF_tw.gif" border=0 alt="關閉快捷鍵"></a></span>
<?php } ?>


<span style="position:absolute;left:0px;top:<?php echo $HKheight ?>px;z-index:15;" id="MaiNfooterspan">
<table BGCOLOR="<?php echo $MAIN_COLOR ?>" id="MaiNfooter" width=<?php echo $MNwidth ?>><tr height=32><td height=32><font face="Arial,Helvetica" size=1>Agent web-client version: <?php echo $version ?> &nbsp; &nbsp; 版次: <?php echo $build ?> &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; 主機: <?php echo $server_ip ?>  &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp;</font><BR>
...
<TR><TD colspan=3><font class="body_text"> 狀態: <span id="MainStatuSSpan"></span></font></TD></TR>
...
<center>
<!-- <a href=\"#\" onclick=\"conf_send_recording('MonitorConf','" + head_conf + "','');return false;\">錄音</a> -->
...
<table><tr>
<td align=right></td>
<td align=left><font class="body_text">&nbsp; Customer Time: <input type=text size=27 name=custdatetime class="cust_form" value=""> &nbsp; 渠道: <input type=text size=27 name=callchannel class="cust_form" value=""></td>
</tr><tr>
<td colspan=2 align=center> 客戶資訊: <span id="CusTInfOSpaN"></span></td>
</tr><tr>
<td align=left colspan=2>


<table width=550><tr>
<td align=right><font class="body_text"> 職銜: </td>
...
<td align=right><font class="body_text">郵遞區號: </td>
...
<td align=right><font class="body_text">性別: </td>
<td align=left><font class="body_text"><span id="GENDERhideFORie"><select size=1 name=gender_list class="cust_form" id=gender_list><option value="U">U - 尚未定義</option><option value="M">M - Male</option><option value="F">F - Female</option></select></span></td>
</tr><tr>
<td align=right><font class="body_text"> 電話: </td>
<td align=left><font class="body_text">
<?php 
