		...
		if (strlen($phone_number)>3)
			...
			#	$DB=1;
				if (strlen($dial_statuses)>2)
					...
					if (eregi("UP LAST 撥號 TIME",$lead_order)){$order_stmt = 'order by last_local_call_time desc, lead_id asc';}
					if (eregi("DOWN LAST 撥號 TIME",$lead_order)){$order_stmt = 'order by last_local_call_time, lead_id asc';}
	...
	if ($VDstop_rec_after_each_call == 1)
		...
		while($loop_count < $total_rec)
			...
			if ($mel > 0) {mysql_error_logging($NOW_TIME,$link,$mel,$stmt,'00098',$user,$server_ip,$session_name,$one_mysql_log);}
				...
				echo "REC_停止|$rec_channels[$loop_count]|$filename[$loop_count]|";
			...
			#			fwrite ($fp, "$NOW_TIME|錄音_LOG|$filename[$loop_count]|$uniqueid|$lead_id|$user|$inOUT|$VLA_inOUT|$length_in_sec|$VDterm_reason|$VDvicidial_id|$VDvicidial_id|$vicidial_id|$start_epoch|$recording_id|$VDIDselect|\n");
			#			fclose($fp);
						}
					else {echo "||";}
					}
...
if ($ACTION == 'VDADcheckINCOMING')
	...
	if ( (strlen($campaign)<1) || (strlen($server_ip)<1) )
		...
		if ($queue_leadID_ct > 0)
			...
			### If 撥號BK, change vicidial_callback record to INACTIVE
...
if ($ACTION == 'userLOGout')
	...
	if ( (strlen($campaign)<1) || (strlen($conf_exten)<1) )
		...
		##### Insert a 登出 record into the user log
		$stmt="INSERT INTO vicidial_user_log (user,event,campaign_id,event_date,event_epoch,user_group) values('$user','登出','$campaign','$NOW_TIME','$StarTtime','$user_group');";
		if ($DB) {echo "$stmt\n";}
		$rslt=mysql_query($stmt, $link);
...
if ($ACTION == 'updateDISPO')
	...
	echo 'Lead ' . $lead_id . ' 已被修改為 ' . $dispo_choice . " 狀態\nNext agent_log_id:\n" . $agent_log_id . "\n";
	}
...
if ($ACTION == 'CALLSINQUEUEview')
	...
	if (eregi('NONE',$view_calls_in_queue))
		...
		echo "<TD BGCOLOR=\"#CCCCCC\"><font style=\"font-size:11px;font-family:sans-serif;\"><B> &nbsp; 撥號 GROUP &nbsp; </font></TD>";
