...
if( (strlen($user)<2) or (strlen($pass)<2) or (!$auth) or ($relogin == 'YES') )
	...
	echo "<title>web client astGUIclient: Início de uma sessão</title>\n";
	echo "</head>\n";
	...
	function liverecording_send_recording(taskvar,taskchan,taskspan,taskfile) 
		...
		if (xmlhttp) 
			...
			if (taskvar == 'StopMonitor')
				...
				filename = taskfile;
				var rec_stop_html = "<a href=\"#\" onclick=\"liverecording_send_recording('Monitor','" + monitorchannelvalue + "','" + taskspan + "','');return false;\">Registro";
	...
	function conf_send_recording(taskconfrectype,taskconfspan,taskconfrec,taskconffile) 
		...
		if (xmlhttp) 
			...
			if (taskconfrectype == 'StopMonitorConf')
				...
				var channelrec = "Local/" + taskconfrec + "@" + ext_context;
				var conf_rec_start_html = "<a href=\"#\" onclick=\"conf_send_recording('MonitorConf','" + taskconfspan + "','" + taskconfrec + "','');return false;\">Registro</a>";
				document.getElementById(taskconfspan).innerHTML = conf_rec_start_html;
				}
					...
					if (live_calls > 0)
						...
						while (loop_ct < live_calls)
							...
							if (recLIST.match(regx)) 
								...
								{live_calls_HTML = live_calls_HTML + "<span id=\"recordlive" + loop_ct + "\"><a href=\"#\" onclick=\"liverecording_send_recording('Monitor','" + channelfieldBtrunk_array[1] + "','recordlive" + loop_ct + "');return false;\">Registro</span>";}
