...
$DefaulTAlTDiaL			= '0';	# set to 1 to enable ALT SELETOR by default if enabled for the campaign
...
echo "<span id=\"LogiNReseT\"><INPUT TYPE=BUTTON VALUE=\"Refresque Campanha List\" OnClick=\"login_allowable_campaigns()\"></span></TD></TR>\n";
...
echo "<TD WIDTH=100 ALIGN=RIGHT VALIGN=TOP  NOWRAP><a href=\"../agc_en/vicidial.php?relogin=YES&VD_login=$VD_login&VD_campaign=$VD_campaign&phone_login=$phone_login&phone_pass=$phone_pass&VD_pass=$VD_pass\">English <img src=\"../agc/images/en.gif\" BORDER=0 HEIGHT=14 WIDTH=20></a></TD>\n";echo "<TD WIDTH=100 ALIGN=RIGHT VALIGN=TOP  BGCOLOR=\"#CCFFCC\" NOWRAP><a href=\"../agc_pt/vicidial.php?relogin=YES&VD_login=$VD_login&VD_campaign=$VD_campaign&phone_login=$phone_login&phone_pass=$phone_pass&VD_pass=$VD_pass\">Português <img src=\"../agc/images/pt.gif\" BORDER=0 HEIGHT=14 WIDTH=20></a></TD>\n";	echo "</TR></TABLE>\n";
	...
	else
	{
		...
		echo "<title>Agent web client: Início de uma sessão</title>\n";
		echo "</head>\n";
		echo "<BODY BGCOLOR=WHITE MARGINHEIGHT=0 MARGINWIDTH=0 onResize=\"browser_dimensions();\"  onLoad=\"browser_dimensions();\">\n";
		echo "<A HREF=\"./timeclock.php?referrer=agent&pl=$phone_login&pp=$phone_pass&VD_login=$VD_login&VD_pass=$VD_pass\">Timeclock</A><BR>\n";
		echo "<TABLE WIDTH=100%><TR><TD></TD>\n";
...
$VDloginDISPLAY=0;
	...
	if($auth>0)
		...
		if ( ( (ereg("COMEÇO|ALL",$shift_enforcement)) and (!ereg("OFF",$VU_agent_shift_enforcement_override)) ) or (ereg("COMEÇO|ALL",$VU_agent_shift_enforcement_override)) )
			{
			$shift_ok=0;
			...
			if ( ($shift_ok < 1) and ($VU_shift_override_flag < 1) and ($VDloginDISPLAY > 0) )
				...
				$VDdisplayMESSAGE.= "<INPUT TYPE=Submit NAME=SUBMETA VALUE=SUBMETA></FORM>\n";
				}
			}
...
echo "<TD WIDTH=100 ALIGN=RIGHT VALIGN=TOP  NOWRAP><a href=\"../agc_en/vicidial.php?relogin=YES&VD_login=$VD_login&VD_campaign=$VD_campaign&phone_login=$phone_login&phone_pass=$phone_pass&VD_pass=$VD_pass\">English <img src=\"../agc/images/en.gif\" BORDER=0 HEIGHT=14 WIDTH=20></a></TD>\n";echo "<TD WIDTH=100 ALIGN=RIGHT VALIGN=TOP  BGCOLOR=\"#CCFFCC\" NOWRAP><a href=\"../agc_pt/vicidial.php?relogin=YES&VD_login=$VD_login&VD_campaign=$VD_campaign&phone_login=$phone_login&phone_pass=$phone_pass&VD_pass=$VD_pass\">Português <img src=\"../agc/images/pt.gif\" BORDER=0 HEIGHT=14 WIDTH=20></a></TD>\n";			echo "</TR></TABLE>\n";
		...
		if($CAMPactive>0)
			...
			if (preg_match("/IN_GROUP|PRESTAURAÇÃO_1|PRESTAURAÇÃO_2|PRESTAURAÇÃO_3|PRESTAURAÇÃO_4|PRESTAURAÇÃO_5/",$quick_transfer_button))
				{$quick_transfer_button_enabled=1;}

			$preset_populate='';
			$prepopulate_transfer_preset_enabled=0;
			if (preg_match("/PRESTAURAÇÃO_1|PRESTAURAÇÃO_2|PRESTAURAÇÃO_3|PRESTAURAÇÃO_4|PRESTAURAÇÃO_5/",$prepopulate_transfer_preset))
				{
				$prepopulate_transfer_preset_enabled=1;
				if (preg_match("/PRESTAURAÇÃO_1/",$prepopulate_transfer_preset))
					{$preset_populate = $xferconf_a_number;}
				if (preg_match("/PRESTAURAÇÃO_2/",$prepopulate_transfer_preset))
					{$preset_populate = $xferconf_b_number;}
				if (preg_match("/PRESTAURAÇÃO_3/",$prepopulate_transfer_preset))
					{$preset_populate = $xferconf_c_number;}
				if (preg_match("/PRESTAURAÇÃO_4/",$prepopulate_transfer_preset))
					{$preset_populate = $xferconf_d_number;}
				if (preg_match("/PRESTAURAÇÃO_5/",$prepopulate_transfer_preset))
...
### logged in and use that phone login account
	...
	if ( ( ($campaign_allow_inbound == 'Y') and ($dial_method != 'MANUAL') ) || ($campaign_leads_to_call > 0) || (ereg('Y',$no_hopper_leads_logins)) )
		...
		echo "<!-- FILA velha e lista reverted INCALL:   |$affected_rows| -->\n";
		...
		$affected_rows = mysql_affected_rows($link);
		echo "<!-- FILA velha e funil reverted INCALL: |$affected_rows| -->\n";
		...
		$affected_rows = mysql_affected_rows($link);
		echo "<!-- os registros velhos dos vicidial_live_agents cancelaram: |$affected_rows| -->\n";
...
// filter manual dialstring and pass em to originate call
	function SendManualDial(taskFromConf)
		...
		if (taskFromConf == 'YES')
			...
			if (three_way_call_cid == 'COSTUMEER')
...
// ZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZ  above section working em API functions
						...
						if ( (conf_channels_xtra_display == 1) || (conf_channels_xtra_display == 0) )
							...
							if (live_conf_calls > 0)
								...
								agentphonelive=0;
								var conv_start=-1;
								var live_conf_HTML = "<font face=\"Arial,Helvetica\"><B>VIVEM AS CHAMADAS EM SUA SESSÃO:</B></font><BR><TABLE WIDTH=<?php echo $CQwidth ?>><TR BGCOLOR=<?php echo $SCRIPT_COLOR ?>><TD><font class=\"log_title\">#</TD><TD><font class=\"log_title\">CANALETA REMOTA</TD><TD><font class=\"log_title\">HANGUP</TD><TD><font class=\"log_title\">VOLUME</TD></TR>";
...
// Covers the following types: XFER, VMAIL, ENTRY, CONF, PARQUE, FROMPARQUE, XfeRLOCAL, XfeRINTERNAL, XfeRBLIND, VfeRVMAIL
		...
		if (xmlhttpXF) 
			...
			if ( (taskvar == 'XfeRBLIND') || (taskvar == 'XfeRVMAIL') )
				...
				if (blindxferdialstring.length<'2')
					...
					xferredirect_query='';
					taskvar = 'NOTHING';
					alert("O número de transferência deve ter mais de 1 dígito:" + blindxferdialstring);
					}
				else
					{
			...
			xmlhttp.onreadystatechange = function() 
				...
				//	alert(xmlhttp.responseText);
					...
					if (CBcounT == 0) {var CBprint = "NO";}
						...
						document.getElementById("CBstatusSpan").innerHTML ="<a href=\"#\" onclick=\"CalLBacKsLisTCheck();return false;\">" + CBprint + " RECHAMADAS ATIVAS</a>";
			...
			if (xmlhttp) 
				...
				xmlhttp.onreadystatechange = function() 
					...
					//	alert(xmlhttp.responseText);
						...
						var loop_ct=0;
						var conv_start=0;
						var CB_HTML = "<table width=610><tr bgcolor=<?php echo $SCRIPT_COLOR ?>><td><font class=\"log_title\">#</td><td><font class=\"log_title\"> CHAMADABACK DATE/TIME</td><td><font class=\"log_title\">NÚMERO</td><td><font class=\"log_title\">NAME</td><td><font class=\"log_title\">  STATUS</td><td align=right><font class=\"log_title\">CAMPAIGN</td><td><font class=\"log_title\">LAST DATE/TIME DA CHAMADA</td><td align=left><font class=\"log_title\"> COMMENTS</td></tr>"
...
// Request lookup of manual dial channel
	function ManualDialCheckChanneL(taskCheckOR)
					...
					//		alert("channel not found yet:\n" + campaign);
						...
						if (taskCheckOR == 'YES')
							...
							if ( (XDchannel.match(regMDL)) && (asterisk_version != '1.0.8') && (asterisk_version != '1.0.9') && (MD_ring_secondS < 10) )
								...
								document.getElementById("MainStatuSSpan").innerHTML = " Ó partido chamado: " + document.vicidial_form.xfernumber.value + " UID: " + CIDcheck;
							...
							if ( (MDchannel.match(regMDL)) && (asterisk_version != '1.0.8') && (asterisk_version != '1.0.9') )
								...
								if (prepopulate_transfer_preset_enabled > 0)
									{
									if (prepopulate_transfer_preset == 'PRESTAURAÇÃO_1')
										{document.vicidial_form.xfernumber.value = CalL_XC_a_NuMber;}
									if (prepopulate_transfer_preset == 'PRESTAURAÇÃO_2')
										{document.vicidial_form.xfernumber.value = CalL_XC_b_NuMber;}
									if (prepopulate_transfer_preset == 'PRESTAURAÇÃO_3')
										{document.vicidial_form.xfernumber.value = CalL_XC_c_NuMber;}
									if (prepopulate_transfer_preset == 'PRESTAURAÇÃO_4')
										{document.vicidial_form.xfernumber.value = CalL_XC_d_NuMber;}
									if (prepopulate_transfer_preset == 'PRESTAURAÇÃO_5')
										{document.vicidial_form.xfernumber.value = CalL_XC_e_NuMber;}
									}
								if ( (quick_transfer_button == 'PRESTAURAÇÃO_1') || (quick_transfer_button == 'PRESTAURAÇÃO_2') || (quick_transfer_button == 'PRESTAURAÇÃO_3') || (quick_transfer_button == 'PRESTAURAÇÃO_4') || (quick_transfer_button == 'PRESTAURAÇÃO_5') )
									{
									...
									if (quick_transfer_button == 'PRESTAURAÇÃO_1')
										{document.vicidial_form.xfernumber.value = CalL_XC_a_NuMber;}
									if (quick_transfer_button == 'PRESTAURAÇÃO_2')
										{document.vicidial_form.xfernumber.value = CalL_XC_b_NuMber;}
									if (quick_transfer_button == 'PRESTAURAÇÃO_3')
										{document.vicidial_form.xfernumber.value = CalL_XC_c_NuMber;}
									if (quick_transfer_button == 'PRESTAURAÇÃO_4')
										{document.vicidial_form.xfernumber.value = CalL_XC_d_NuMber;}
									if (quick_transfer_button == 'PRESTAURAÇÃO_5')
										{document.vicidial_form.xfernumber.value = CalL_XC_e_NuMber;}
									}
		...
		if (MD_ring_secondS > 49) 
			...
			MD_channel_look=0;
			MD_ring_secondS=0;
			alert("O seletor cronometrado para fora, contata seu administrador de\n");
			}
...
// Send the Manual Número Seguinte Do Seletor request
	function ManualDialNext(mdnCBid,mdnBDleadid,mdnDiaLCodE,mdnPhonENumbeR,mdnStagE,mdVendorid,mdgroupalias)
		...
		if (document.vicidial_form.LeadPreview.checked==true)
			...
			var man_preview = 'YES';
			var man_status = "Inspecione a ligação então <a href=\"#\" onclick=\"ManualDialOnly()\"><font class=\"preview_text\"> LIGAÇÃO DO SELETOR</font></a> or <a href=\"#\" onclick=\"ManualDialSkip()\"><font class=\"preview_text\">LIGAÇÃO DA FAIXA CLARA</font></a>"; 
			}
		else
			{
...
// Send the Manual Dial Skip
	function ManualDialSkip()
		...
		if (manual_dial_in_progress==1)
			{
			alert('VOCÊ NÃO PODE SALTAR Uma RECHAMADA OU SELETOR MANUAL, VOCÊ DEVE MARCAR A LIGAÇÃO');
			}
		else
			{
			if (dial_method == "INBOUND_MAN")
				...
				xmlhttp.onreadystatechange = function() 
					...
					//	alert(xmlhttp.responseText);
						...
						if (MDSnextCID == "LEAD NOT REVERTED")
							{
							alert("A ligação não reverted, não havia um erro:\n" + MDSnextResponse);
							}
						else
							{
...
// Send the Manual Dial Only - dial the previewed lead
	function ManualDialOnly(taskaltnum)
		...
		if (dialed_label == 'ADDR3')
			{document.getElementById("CusTInfOSpaN").innerHTML = " <B> ALT PROGR.SELEC NÚMERO: ENDEREÇO3 </B>";}
...
// Generate the Grupo Alias Chooser panel
	function GroupAliasSelectContent_create(task3way)
		...
		if (task3way > 0)
			...
			GroupAlias_HTML = GroupAlias_HTML + "<font size=2 style=\"BACKGROUND-COLOR: #FFFFCC\"><b><a href=\"#\" onclick=\"GroupAliasSelect_submit('COSTUMEER','" + document.vicidial_form.phone_number.value + "','0');return false;\">COSTUMEER - " + document.vicidial_form.phone_number.value + "</a></b></font><BR><BR>";
...
// open web form, then submit disposition
	function WeBForMDispoSelect_submit()
		...
		if (DispoChoice.length < 1) {alert("Você deve selecionar uma disposição");}
		else
			{
...
// ################################################################################
// Refresque the agents view sidebar or xfer frame
...
// ################################################################################
// Refresque the calls in queue bottombar
	...
	function clearDiv(divvar)
		...
		if (document.getElementById(divvar))
			...
			if (divvar == 'DiaLDiaLAltPhonE')
				{
				var buildDivHTML = "<font class=\"preview_text\"> <input type=checkbox name=DiaLAltPhonE size=1 value=\"0\"> ALT DO TELEFONE<BR></font>";
		...
		if (VU_vicidial_transfers == '1')
			...
			if (showxfervar == 'ON')
				...
				if (showoffvar == 'YES')
					...
					if ( (quick_transfer_button == 'PRESTAURAÇÃO_1') || (quick_transfer_button == 'PRESTAURAÇÃO_2') )
						{
						document.getElementById("QuickXfer").innerHTML = "<a href=\"#\" onclick=\"mainxfer_send_redirect('XfeRBLIND','" + lastcustchannel + "','" + lastcustserverip + "');return false;\"><IMG SRC=\"../agc/images/vdc_LB_quickxfer.gif\" border=0 alt=\"RÁPIDA TRANSFERÊNCIA\"></a>";
						}
					}
			...
			if (showxfervar != 'OFF')
				{
				alert('VOCÊ NÃO TEM PERMISSÕES TRANSFERIR CHAMADAS');
				}
			}
		}
...
<?php	echo "Usuário: $VD_login em Telefone: $SIP_user à campanha: $VD_campaign&nbsp; \n"; ?>
...
<span style="position:absolute;left:0px;top:0px;z-index:3;" id="Boa vindaBoxA">
    <table border=0 bgcolor="#FFFFFF" width=<?php echo $CAwidth ?> height=<?php echo $HKwidth ?>><TR><TD align=center><BR><span id="Boa vindaBoxAt">Agent Screen</span></TD></TR></TABLE>
</span>
...
<span style="position:absolute;left:0px;top:0px;z-index:51;" id="CallBacKsLisTBox">
	...
	<a href="#" onclick="CalLBacKsLisTCheck();return false;">Refresque</a>
	 &nbsp;  &nbsp;  &nbsp;  &nbsp;  &nbsp;  &nbsp;  &nbsp;  &nbsp; 
	<a href="#" onclick="hideDiv('CallBacKsLisTBox');return false;">Vá Para trás</a>
	</TD></TR></TABLE>
</span>
...
<span style="position:absolute;left:0px;top:0px;z-index:52;" id="NeWManuaLDiaLBox">
	...
	<td align=right><font class="body_text"> Ultrapassagem Do Seletor: </td>
	<td align=left><font class="body_text"><input type=text size=24 maxlength=20 name=MDDiaLOverridE class="cust_form" value="">&nbsp; (os dígitos satisfazem somente)
	</td>
	</tr></table>
	<BR>
	...
	<a href="#" onclick="NeWManuaLDiaLCalLSubmiT('PREVIEW');return false;">Preview Call</a>
	 &nbsp;  &nbsp;  &nbsp;  &nbsp;  &nbsp;  &nbsp;  &nbsp;  &nbsp; 
	<a href="#" onclick="hideDiv('NeWManuaLDiaLBox');return false;">Vá Para trás</a>
	</TD></TR></TABLE>
</span>
...
<span style="position:absolute;left:0px;top:0px;z-index:48;" id="DispoButtonHideC">
    <table border=0 bgcolor="#CCFFCC" width=<?php echo $CAwidth ?> height=47><TR><TD align=center VALIGN=top>Todas as mudanças feitas ao cliente que a informação abaixo neste tempo não será comitted, você devem mudar a informação do cliente antes de você hangup a chamada. </TD></TR></TABLE>
</span>
...
<span style="position:absolute;left:0px;top:0px;z-index:49;" id="DispoSelectBox">
	...
	<BR><BR>
	<a href="#" onclick="WeBForMDispoSelect_submit();return false;">WEB FORM SUBMETA</a>
	<BR><BR> &nbsp; 
	</TD></TR></TABLE>
</span>
...
<span style="position:absolute;left:0px;top:0px;z-index:55;" id="NothingBox">
	...
	if ( ($outbound_autodial_active < 1) or ($disable_blended_checkbox > 0) or ($dial_method == 'INBOUND_MAN') )
		{echo "<input type=checkbox name=CloserSelectBlended size=1 value=\"0\"> CHAMADA MISTURADA<BR>";}
	?>
</span>
...
<span style="position:absolute;left:<?php echo $HKwidth ?>px;top:<?php echo $HKheight ?>px;z-index:16;" id="hotkeysdisplay"><a href="#" onMouseOver="HotKeys('ON')"><IMG SRC="../agc/images/vdc_XB_hotkeysactive_OFF.gif" border=0 alt="CHAVES QUENTES INATIVAS"></a></span>
<?php } ?>


<span style="position:absolute;left:0px;top:<?php echo $HKheight ?>px;z-index:15;" id="MaiNfooterspan">
<table BGCOLOR="<?php echo $MAIN_COLOR ?>" id="MaiNfooter" width=<?php echo $MNwidth ?>><tr height=32><td height=32><font face="Arial,Helvetica" size=1>Agent web-client version: <?php echo $version ?> &nbsp; &nbsp; CONFIGURAÇÃO: <?php echo $build ?> &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; Usuário: <?php echo $server_ip ?>  &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp;</font><BR>
...
<span id="DiaLDiaLAltPhonE"><font class="preview_text"> <input type=checkbox name=DiaLAltPhonE size=1 value="0"> ALT DO TELEFONE<BR></font></span>
...
if ( ($alt_phone_dialing) and ($auto_dial_level==0) )
	{echo "<font class=\"preview_text\"> <input type=checkbox name=DiaLAltPhonE size=1 value=\"0\"> ALT DO TELEFONE<BR></font>";}
...
ID REGISTRO: <font class="body_small"><span id="RecorDID"></span></font><BR>
<center>
<!-- <a href=\"#\" onclick=\"conf_send_recording('MonitorConf','" + head_conf + "','');return false;\">Registro</a> -->
...
<table><tr>
<td align=right></td>
<td align=left><font class="body_text">&nbsp; Customer Time: <input type=text size=27 name=custdatetime class="cust_form" value=""> &nbsp; Canaleta: <input type=text size=27 name=callchannel class="cust_form" value=""></td>
</tr><tr>
<td colspan=2 align=center> Informação Do Cliente: <span id="CusTInfOSpaN"></span></td>
</tr><tr>
<td align=left colspan=2>
...
<td align=right><font class="body_text">Alt. Telefone: </td>
